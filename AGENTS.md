...
Single repo, flat layout: `github.com/stewie-sw/stewie`. Two components:

| Component | Path | What it is |
|---|---|---|
| **terrain_authority** (core) | `terrain_authority/` | Conserved Tier-2 terramechanics authority + RL envs + the **stewie** Gymnasium suite + drum-mass sensing. NumPy-only hot path. |
| **planet_browser** (product) | `planet_browser/` | The SimCity-style build planner: CesiumJS globe + build-order queue → `mission_planner` → 2-3pp mission-control PDF. FastAPI-free (stdlib `http.server`); installed as a package (entry point `stewie-serve`). |
...
# the product (web app): then open the printed URL  (or use the `stewie-serve` entry point)
...
# the stewie Gymnasium suite
PYTHONPATH=. $VENV -c "import stewie, gymnasium as gym; print(gym.make('Stewie/Scheduler-v0'))"
```
...
- `bodies.py` / `registration.py` — per-planet constants (sysrev-sourced) + the `Stewie/*` env IDs.
...
## Current state
Single repo `github.com/stewie-sw/stewie` (flat layout; the old `roversim/` split is gone). CI runs the
suite on every push. stewie is **not yet on PyPI** (publish is owner-gated). Forward plan + deep status
