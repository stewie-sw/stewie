...
| `physics.energy_cost` | raster | steady drive power (W), not an energy total -- legacy id (task #53) | derived/forecast | yes | yes if calibrated |
| `physics.excavation_resistance` | raster | dig draft force from the McKyes/Reece earthmoving equation over the bucket-drum cut (task #78) | derived/estimated | yes | yes if calibrated |
