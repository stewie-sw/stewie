...
from stewie.specs import constants as _K  # noqa: E402
...
def _o(action, kind, x, y, footprint_m2, depth_m, note=""):
    ...
    o = {"action": action, "kind": kind, "x": float(x), "y": float(y),
         "footprint_m2": float(footprint_m2), "depth_m": float(depth_m), "note": note}
    # task #78 reconciliation: a structure CUT excavates engineered BANK (in-situ) material -- the density
    # this module sizes the fill by (SWELL = RHO_BANK/RHO_LOOSE) AND the density the conserved column_state
    # authority + planner_acceptance + constructability + siteplan all cost the cut at. Declaring it
    # explicitly makes lode.planner_balance (which since #78 derives a depth-averaged per-cut density, so a
    # shallow structure cut would otherwise read as ~RHO_LOOSE and mis-balance) cost the cut at the SAME
    # RHO_BANK the rest of the conserved ecosystem uses -- so the structure balances in the planner without
    # shrinking the fill (which would break the flat-density conserved-authority validate_plan). See
    # BuildOrder.insitu_density_kg_m3 / lode.planner_balance.cut_bank_density.
    if kind == "cut":
        o["insitu_density_kg_m3"] = RHO_BANK
    return o


# -- balanced structures: the fill consumes exactly the paired cut (volume-conserved). Paired-site
# separation is FOOTPRINT-SCALED (half cut extent + half fill extent + 2 m margin): the old fixed
...
def landing_pad(x, y, *, side_m=6.0, cut_depth_m=0.05, berm_height_m=0.12):
    ...
    return [
        ...
        _o("Perimeter blast berm", "fill",
           x + side_m / 2 + math.sqrt(fill_vol / berm_height_m) / 2 + 2.0, y,
           fill_vol / berm_height_m, berm_height_m, "from pad cut (bulked)"),
    ]
...
def habitat_foundation(x, y, *, side_m=5.0, cut_depth_m=0.06, fill_height_m=0.06):
    ...
    return [
        ...
        _o("Compacted foundation fill", "fill",
           x + side_m / 2 + math.sqrt(fill_vol / fill_height_m) / 2 + 2.0, y,
           fill_vol / fill_height_m, fill_height_m, "from footing cut (bulked)"),
    ]
...
def blast_berm(x, y, *, length_m=15.0, width_m=3.0, height_m=0.5, borrow_depth_m=0.3):
    ...
    return [
        _o("Borrow pit (berm)", "cut",
           ...
           borrow_vol / borrow_depth_m, borrow_depth_m, "source"),
