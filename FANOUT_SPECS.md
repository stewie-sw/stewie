...
### TR-02 (P1) — atomic
- goal: An INTERPRETABLE search-distilled excavation/construction policy: a depth-limited decision tree over PHYSICAL features, distilled from model-based search run in the exact conserved sim on frozen scenarios (TR-01). NOT a black box, and NOT model-free RL -- this repo's own evidence says model-based search + distillation is the lever (beam 24 legs > PPO 27 > greedy 28; the search-distilled MLP hit the optimum). A tree is the same proven pattern with a READABLE target.
- acceptance: beam/CEM search in the exact authority yields optimal maneuver labels WITHOUT human demonstrations; distill into a depth-limited decision tree over features that are ALREADY real telemetry (avg_slope_deg, drum_fill_frac, dig_j_per_kg [FEE, prop depth^2], dig_reaction_n, dig_engagement, inventory_kg, odometry_error_m + per-cell derive_height/diff_field/cell_strength/bearing/wheel_static_sinkage/slip, AND the PX-14 drum-current observable); the tree must BEAT GREEDY on passes-to-spec / energy-to-spec / as-built RMSE (+/-2 cm), scored against the beam optimum. GENERALISATION on HELD-OUT rock seeds (train 0..N, eval unseen draws -- possible only because TR-01 made rock_seed live). Engineer the physics INTO the feature (hand it predicted_fee_j_per_kg(candidate_bite), do not make the tree rediscover energy prop depth^2).
- THE NULL RESULT IS PRE-COMMITTED: a TIE with greedy is a NULL RESULT and MUST be reported as one. The prior finding that greedy AND RANDOM both hit 100% on the unconstrained flatten is exactly the trap this row exists to avoid; RL/search only earns its keep under real constraints (drum capacity, energy, metered dump, the two-drum reaction).
- current_state: the ORACLE is cheap (the conserved sim is mass-exact + sub-ms), the FEATURES now all EXIST (PX-08..PX-14 made slope/slip, FEE energy, drum fill, bite cap, metered discharge, two-drum reaction, and the drum-current observable real + live), and TR-01 froze the scenarios + made the seed live for the train/eval split. This path does NOT need human demonstrations -- the oracle is the SEARCH -- so it is NOT gated on the observation-paired recorder (that remains necessary only for distilling OPERATOR intuition where the cost function is wrong: slumping, rock avoidance, approach angle).
- PRIOR ART (do not overclaim): NTRS 20210022218 (ICE-RASSOR *learning excavation*) is NASA replacing the AutoDig PID trencher with deep RL. The delta here is (a) an INTERPRETABLE policy and (b) model-based distillation.
- files: NEW stewie/dataset/ or leap/ search+distill module + a demo; reuses stewie/runtime/viz2_runtime.py (features), stewie/dataset/scenario.py (frozen worlds)
- test_target: NEW [REQ:TR-02] gate: beam beats greedy on a frozen scenario; the distilled tree matches beam within tolerance and BEATS greedy (or the null result is reported); held-out-seed generalisation.
...
### GW-13 — atomic
- goal: Rebase viz2's Godot scene to a LOCAL render origin so it honours the GW-12 local-render-origin rule, which it currently breaches: it places nodes at ABSOLUTE lunar coordinates, so float32 render precision is site-dependent.
- acceptance: every site viz2 serves clears GW-12's documented float32 budget (ULP < 1 cm). Render-origin-relative, scene coords span only the site extent (<= ~20 km -> ULP <= 1.2 mm; 0.12 mm over a 2 km window) -> 8x+ margin everywhere. The [REQ:GW-13] canary's breach set becomes EMPTY and must be asserted as set() when the fix lands. Gate the change on a BEFORE/AFTER RENDER CAPTURE, not just the parse gate.
- current_state: BREACHED and MEASURED. state_fields.gd:152 sets world_min to the bundle's real IAU_2015:30135 corner and _pose_x = world_min.x + col*cell_m flows straight into float32 Node3D transforms. malapert_massif_10km_5m (|132 km| -> 15.62 mm ULP) and nobile_rim2_10km_5m (|164 km| -> 15.62 mm) BREACH the 1 cm bound -- coarser than the sim's own 13.2 mm anti-bridging bite; nine more sites sit at only 1.28-2.56x margin. Render-only severity (the authoritative pose is float64 in the runtime and is what the HUD reports; Godot never sends a world coord back; both breaching bundles are 5 m-cell DEMs). GW-12's own test does not cover the Godot path. ATTEMPTED 2026-07-13 AND REVERTED: terrain + rover rendered correctly, the Godot --check-only parse gate PASSED, the HUD coord stayed exactly right, zero console errors -- and the ENTIRE ROCK FIELD still vanished (clasts arrive from the runtime's clasts.json in absolute world metres and did not land in the local frame). Only a screenshot caught it. RETRY MUST first establish frame-ownership for EVERY world-coordinate ingest (clasts.json, the {plan:{route}} push, the lander/boulder-manifest path) and gate on a render capture. This is a LIVE PUBLIC surface (RT-06) -> wants a human in the loop; rollback is `git checkout -- stewie/godot/ && systemctl --user restart viz2-stream` (proven).
- files: stewie/godot/state_fields.gd, stewie/godot/viz2_root.gd, stewie/stream/app.py, stewie/runtime/viz2_runtime.py
- test_target: stewie/dataset/test_float32_render_bound.py
