...
# Committed real Stewie 1024x768 capture (no synthetic data).
...
@pytest.mark.skipif(not _has, reason="stewie sensors.json fixture not present")
def test_read_real_sensors_frame():
    f = sensor_io.read_sensors(REAL_SENSORS)
    ...
    assert f.profile_id == "STEWIE_IPEX_V1"
...
def test_truth_firewall_sensorframe_carries_no_truth():
    ...
    truth = sensor_io.read_evaluation_truth(REAL_TRUTH)
...
def test_load_real_camera_image():
    img = sensor_io.load_camera_image(REAL_SENSORS, "front_left")
...
def test_legacy_combined_packet_is_rejected():
    ...
    with pytest.raises(sensor_io.PacketValidationError, match="runtime_sensors.json"):
        sensor_io.read_sensors(legacy)
...
def test_runtime_truth_injection_is_rejected(tmp_path):
    ...
    with pytest.raises(sensor_io.PacketValidationError, match="evaluation-only"):
        sensor_io.read_sensors(str(path), validate_images=False)
...
def test_bad_profile_checksum_is_rejected_by_profile_validator():
    ...
    frame = sensor_io.read_sensors(REAL_SENSORS)
...
def test_write_cmd_vel_roundtrip(tmp_path):
    p = sensor_io.write_cmd_vel(str(tmp_path), 0.2, -0.1, 7)
    d = json.load(open(p))
    assert d["v_ms"] == 0.2 and d["omega_rads"] == -0.1 and d["frame_index"] == 7


def test_write_posture_command_roundtrip(tmp_path):
    p = sensor_io.write_posture_command(str(tmp_path), 1.2, 0.0, posture="MEERKAT",
                                         drum_front=0.5)
    d = json.load(open(p))
