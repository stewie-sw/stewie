// FS-03 (node:test): the Models-pane renderers are pure -> unit-testable without a browser.
...
test("modelsProfilesHTML: renders the system-profile registry with status + sha256", () => {
  const html = M.modelsProfilesHTML(MODELS, esc);
...
test("modelsProfilesHTML: honest empty when no profiles", () => {
  assert.ok(M.modelsProfilesHTML({ profiles: [] }, esc).includes("No profiles served"));
});

test("modelsRegistriesHTML: renders the vehicle + body registries with provenance", () => {
  const html = M.modelsRegistriesHTML(MODELS, esc);
...
test("modelsRegistriesHTML: honest empty when no registries", () => {
  assert.ok(M.modelsRegistriesHTML({ vehicles: [], bodies: [] }, esc).includes("No registries served"));
});

test("modelsGovernanceHTML: renders ML-01 gate + the no-command-path status", () => {
  const html = M.modelsGovernanceHTML(MODELS, esc);
