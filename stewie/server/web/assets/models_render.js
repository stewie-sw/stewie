...
// lookups, no network, no module globals -- the cockpit's thin wrapper fetches /models and passes the
// payload in, mirroring fleet_render.js. Builders:
...
(function (root) {
  ...
  function modelsProfilesHTML(m, esc) {
    ...
    var rows = m.profiles.map(function (p) {
      ...
      return "<tr>"
        ...
        + '<td style="text-align:right;font-variant-numeric:tabular-nums">' + esc(p.n_cameras) + "</td>"
        + '<td style="text-align:right;font-variant-numeric:tabular-nums">'
          + (p.dry_mass_kg != null ? Number(p.dry_mass_kg).toFixed(1) : "—") + "</td>"
    ...
    return '<table style="width:100%;border-collapse:collapse;font-size:11px">'
      ...
      + esc(m.profile_count || 0) + " profiles · " + esc(m.profiles_deployable || 0)
      + " VERIFIED (deployable) · default " + esc(m.default_profile || "") + "</div>";
  }
  ...
  function modelsRegistriesHTML(m, esc) {
    ...
    var brows = bod.map(function (b) {
      return "<tr>"
        ...
        + "<td>" + esc(b.bekker_regime) + "</td>"
        + '<td style="text-align:right;font-variant-numeric:tabular-nums">' + Number(b.g_m_s2 || 0).toFixed(2) + "</td>"
    ...
    if (veh.length) {
      html += '<div style="font-family:Orbitron,system-ui;font-size:10px;letter-spacing:.08em;'
        ...
        + esc(m.vehicle_count || veh.length) + ", default " + esc(m.default_vehicle || "") + ")</div>"
    ...
    if (bod.length) {
      html += '<div style="font-family:Orbitron,system-ui;font-size:10px;letter-spacing:.08em;'
        ...
        + esc(m.body_count || bod.length) + ", default " + esc(m.default_body || "") + ")</div>"
  ...
  function modelsGovernanceHTML(m, esc) {
    var g = (m && m.model_governance) || {};
    var crit = (g.deployment_ready_criteria || []).map(function (c) {
      return '<li style="font-size:11px;opacity:.9">' + esc(c) + "</li>";
    }).join("");
    var deployed = Array.isArray(g.deployed_models) ? g.deployed_models : [];
    var enforced = g.command_path_enforced
    ...
    return '<div style="font-size:11px;line-height:1.6">'
      ...
      + esc(g.schema_endpoint || "/contracts/schema") + "</div>"
      ...
      + ' — <span style="opacity:.85">' + esc(g.command_path_invariant || "") + "</span></div>"
