"""[REQ:GW-00] the GeoLibre map + RViz/Godot web-panel CSP contract (deploy/nginx.conf, the sole CSP surface —
the backend sets none). The production CSP allowlists the NASA Trek tile host for BOTH img-src and connect-src
so MapLibre/GeoLibre streams real Trek raster tiles, permits MapLibre's blob web workers (worker-src blob:),
...
def test_gw00_csp_allowlists_trek_for_geolibre_imagery():  # [REQ:GW-00]
    csp = _csp_directives()
    for d in ("img-src", "connect-src"):
        assert "https://trek.nasa.gov" in csp.get(d, []), \
            f"CSP {d} must allowlist https://trek.nasa.gov so the GeoLibre basemap streams real Trek tiles"


def test_gw00_csp_supports_maplibre_workers_and_web_panels():  # [REQ:GW-00]
    csp = _csp_directives()
    assert "blob:" in csp.get("worker-src", []), "MapLibre needs blob web workers (worker-src blob:)"
