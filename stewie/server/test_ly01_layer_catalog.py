...
def test_ly01_catalog_json_is_in_sync_with_the_prd2_table():  # [REQ:LY-01]
    ...
    assert committed["count"] == live["count"] == 68   # 65 + traffic.compaction (TW-11) + LY-05 terrain.aspect + terrain.curvature
...
def test_ly01_endpoint_serves_the_catalog_with_eligibility_rules(monkeypatch):  # [REQ:LY-01]
    ...
    assert j["count"] == 68 and len(j["layers"]) == 68   # 65 + traffic.compaction (TW-11) + LY-05 aspect + curvature
