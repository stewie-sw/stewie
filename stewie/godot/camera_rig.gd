...
const BASELINE_M := 0.05               # TRL5-FINAL flight baseline (SCHULER24 Figs 28/30/32: the stereo pair
                                       # combined into a single rigid housing, replacing the rejected 0.165 m
                                       # split). Adopted 2026-06-18 (re-froze the G2 runtime_sensors corpus).
const INITIAL_BASELINE_M := 0.165      # [SCHULER24 Fig.28 left] real split-shoulder INITIAL design (rejected; reference only)
...
const REAR_BASELINE_M := 0.05          # rear module mirrors the front 0.05 m rigid housing (see BASELINE_M)
