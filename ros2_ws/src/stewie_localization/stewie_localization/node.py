"""stewie_localization (localization) -- AS-02 Phase-1 skeleton node. Conforms to the AS-01 boundary contract
...
def main(args=None):
    ...
    node.get_logger().info("stewie_localization up (AS-02 skeleton; role=localization)")
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
