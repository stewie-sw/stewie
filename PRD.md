...
| MG-02 | P0 | RETIRED: the per-pane vanilla->React flip-with-parity-gates this row governs was never adopted (no /app2). The product front door is the QWC2 /ide (b833069); the React frontend/ cockpit is a separate surface, not a pane-by-pane flip of the vanilla cockpit. | N | N | N | NA |
...
| RT-02 | P0 | **Evidence bound to run/profile.** ROS/Gazebo/RViz/Godot evidence surfaces attach to the selected run/profile and hold no independent command authority; the execution service is the sole command egress. (extends BA-08) | N | N | N | NA |
