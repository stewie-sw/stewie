...
- **Adds:** ✅ the map scorer + the second eval channel + a producer-independent real-DEM ingress + ✅ the ONBOARD rover-stereo observed-map producer. ⬜ the GROUND COLMAP producer scored on the mission moving-sequence (the high-accuracy tier; `scripts/colmap/` is self-verified at 12/12, 0.296 px reproj, ATE 5.7 mm but not yet scored vs truth). Onboard (cheap, real-time, noisy) and ground COLMAP (offline, accurate) are complementary tiers; the sim feeds and grades both against the same conserved truth.
