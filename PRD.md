...
| O4 | P2 | **Per-vehicle config** — a vehicle/rover spec object (not global `ipex_specs` singletons) so different rovers carry different specs (ties to MV6 heterogeneous fleet). | 🟡 `vehicles.py`: `Vehicle`/`PowerSource`/`Tool` registries + `PowerGrid` (N:N power↔vehicle) + `Placement`/`Deployment` (assign vehicle+tools+power to a **body**); `ipex` reproduces today's numbers; sinter is a separate `Tool`, not an `ipex` capability; capabilities + fleet exported to `bodies.json` for the browser (`gen_bodies_json`). Capability-gating LIVE: `mission_from_dict` gates order kinds by the selected vehicle+tools (sinter = capability-gate + the SINTER_ENABLED numbers-flag); the browser renders vehicle/tool pickers + a capability-gated kind dropdown and sends `vehicle`/`tools` in `/plan`. **Soil/gravity decoupled from the body** (the `Body`'s world-vs-regolith roles split): a `soil` override (any body's regolith) + a `g` override let you run e.g. Earth terramechanics on a lunar map — wired through `mission_from_dict.soil` → the planner slip (`mission_soil_params` → `slip_alpha_to_slip(params=)`), `RoverSimEnv(soil=)`, `Placement.soil`/`g` + `Deployment.params_for`/`gravity_for`, and a browser "Soil" dropdown (`test_soil_override.py`). **Per-vehicle GEOMETRY + two selectable bodies (2026-06-06):** `Vehicle` now carries `gauge_m`/`wheelbase_m`/`wheel_radius_m`/`cg_height_m`/`render_assets`; **`ez_rassor`** (MIT EZ-RASSOR URDF stance = the byte-identical default) and **`ipex`** (flight-scale: wheel r 0.1524 sourced, track 0.7×RASSOR-2, wheelbase/CG [CALIB]; render = the **CC0 self-authored** primitive `scripts/gen_ipex_mesh.py` → `assets/ipex/`) are both registered and selectable at every stage — `geometry_of()` → `stability.stability` kwargs, `RoverSimEnv(vehicle=)` (tip-over physics), `bodies.json _vehicles[]` (browser), and the Godot sidecar (`--rover-assets`/`--rover-gauge`/`--rover-wheelbase`, defaults = EZ-RASSOR). Energy/drum/terramechanics shared (IPEx-grounded); the bodies differ in geometry→tip-over + render mesh. **Both bodies headless-render-verified** (Godot 4.6.3 + xvfb + Vulkan, RTX 3090): EZ-RASSOR mesh AABB 1.83×0.66×1.70 m, CC0 IPEx primitive AABB 1.26×0.58×0.99 m (`godot_sidecar/out/body_{ez_rassor,ipex_cc0}.png`). **Remaining:** thread a *second* vehicle's mass/energy through the planner so the plan NUMBERS change per vehicle (B.2) — still needs a sourced second energy spec (no fabrication). |
