...
| PO-15 | P1 | **Operations governance beyond account admin (review finding 7).** Backup/restore is a scheduled + monitored control with an explicit RPO/retention policy (not a manual director endpoint at `admin_ops.py:18`); mission, runtime, and profile administration exist as first-class operations surfaces (not browser-local display preferences at `index.html:1354`). Acceptance: a retention/RPO policy is declared and enforced by a scheduled job with a monitored last-success/age signal; mission/runtime/profile admin actions are audited. Buildable on archimedes. | P | N | N | NA |
...
| FS-26 | P2 | **Public /program board fits the mobile viewport (review finding 9).** The `/program` board renders within the viewport at 390 px with no horizontal overflow (today content is ~575 px wide -> horizontal scroll; `program.html:28`). Acceptance: a Playwright check at 390 px asserts `document.scrollingElement.scrollWidth <= innerWidth` and every row/table scrolls inside its own container, not the page body. Buildable on archimedes. | N | N | N | NA |
