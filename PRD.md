...
| ID | P | Requirement and acceptance | I | X | V | Q |
|---|---|---|---|---|---|---|
| GW-00 | P0 | **Prerequisite — Trek imagery + web-panel CSP allowlist.** `deploy/nginx.conf` + backend CSP allow `trek.nasa.gov` (and rosbridge/foxglove origins) so GeoLibre streams real NASA Trek tiles and RViz/Godot web panels load; a test asserts the CSP header lists the allowed hosts. Blocks GW-05, RT-04. | N | N | N | NA |
...
| GW-06 | P0 | **Layer tree + legend.** The LY-01 catalog toggles on the map with per-layer swatch + provenance/freshness; a display-only layer is visibly distinct from a planning/release/execute-eligible one. (binds LY-01) | N | N | N | NA |
| GW-07 | P0 | **Selection + right inspector.** Click a cell/feature → attributes + provenance + confidence/freshness + available actions + runtime evidence affecting it. (extends GL-02) | N | N | N | NA |
...
| GW-02 | P0 | **Unified workspace context.** One routeable context (the PRD2 field set: mission/site/body/CRS/local-frame/vehicle/fleet/selection/layers/mode/profile/source/namespace/backend/depth/time/branch/release/run/role) drives map/assets/Godot/RViz/Gazebo-evidence/report/authority; one URL restores all of it. (extends RF-02) | N | N | N | NA |
| GW-03 | P0 | **Layer-eligibility UI.** The layer manifest UI differentiates display/planning/release/execute eligibility + exposes freshness/provenance/uncertainty; a display-only layer cannot be marked planning-valid. (extends DW-01; binds LY-01) | N | N | N | NA |
| GW-04 | P1 | **Asset Library.** Browse/search/inspect/export/recover durable assets (missions/sites/DEMs/layer-packages/vehicles/plans/rehearsals/runs/reports/evidence/bags/captures), separate from visible layers; every durable object traces to provenance. | N | N | N | NA |
...
| LY-02 | P1 | **Layer-consumption inspector.** For a selected layer, show where it is consumed: display / planner / costmap / rehearsal / release / execute / report / export. | N | N | N | NA |
| PH-01 | P0 | **Physics backend registry.** Exposes per backend (tier2_numpy/gazebo/chrono/hardware/godot) the authority scope (terrain/robot/sensor/contact/rendering/hardware), mass-conservation, calibration id, per-profile validity (planning/rehearsal/release/execute), refusal reasons, and evidence artifacts. (extends PX-02) | N | N | N | NA |
| PH-02 | P1 | **Physics attribution.** The planner + reports attribute every route/volume/risk value to a physics backend + calibration source; a value with no backend attribution is not release-eligible. | N | N | N | NA |
| TM-02 | P0 | **Terramechanics spine (inspectable terms).** Slope/roughness/regolith/bearing/sinkage/slip/traction-margin/excavation-resistance/energy/uncertainty are exposed as inspectable terms, each classified simulated/sensed/estimated/learned/assumed; a route cost exposes its terms, not only a scalar. (complements the TM-01 calibration gate) | N | N | N | NA |
| TM-03 | P1 | **Terramechanics output layers.** Terramechanics generates traversability/energy/slip-risk/bearing-sinkage/excavation-feasibility/constructability layers into the LY-01 catalog with source-class + uncertainty. | N | N | N | NA |
| TM-04 | P1 | **Predicted-vs-observed terramechanics.** A rehearsal/report compares predicted slip/sinkage/energy against observed or replayed telemetry when available; as-built excavation updates terrain memory + future cost layers. | N | N | N | NA |
| SD-01 | P1 | **Surface Design view.** Pads/berms/trenches/cuts/fills/roads/stockpiles/sinter → typed mission orders with volume/mass/bearing/sinkage/constructability evidence; volume/mass estimates show DEM resolution, material assumption, and uncertainty; a before/after terrain transaction influences the next plan. | N | N | N | NA |
| RT-01 | P0 | **Runtime profile registry.** A GET endpoint defines desktop_sil/digital_twin/ros2_replay/gazebo_sim/hil/field_test/live_rover and each profile's allowed command + evidence capabilities; the cockpit binds it. (extends PX-02/FR-01; names gazebo_sim) | N | N | N | NA |
...
| AU-01 | P0 | **Global command-authority card.** Visible from every command-capable view whenever command capability exists, showing every refusal reason; a disabled control names why. (extends FR-03) | N | N | N | NA |
