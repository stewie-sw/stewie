...
| TW-04 | P1 | One seeded composite generator combines craters, rocks, material, and illumination parameters. `stewie/terrain/test_scenes.py` [REQ:TW-04] validates `build_from_dem` combining all four generators + provenance + datum round-trip on the real Haworth DEM. V held at P pending an I=D implementation-completeness audit (the citing test is genuine — this row is "understated / under review for promotion", not silently downgraded). | P | P | P | NA |
...
| CP-04 | P1 | Goal grammar supports typed structures, tolerances, budgets, priorities, deadlines, dependencies, and keep-outs. `lode/mission_intent_compiler.py` lowers a canonical MO-01 `MissionIntent` onto the real planner path: objectives->`cut` orders, hard energy/time budgets->`objective_constraints`, soft-weight priorities->the weighted objective, deadlines->`max_time_s`, prerequisites->precedence, TOLERANCES (`AcceptanceCriterion.tolerance_m`, tightest per mission->`Mission.accept_flatness_tol_m`, honored by `validate_plan`'s as-built RMSE gate), and KEEP-OUTS (`KeepOutRegion` circle/rectangle/polygon->`Mission.keepouts`, the planner's own routing input: `route_leg` detours around them, a build inside is flagged `keepout_conflicts`). Wired into the product path via `lode/mission_lifecycle.analyze`/`rehearse` (executive ANALYZED/REHEARSED compile the intent to the planner `Mission`); the compiled tolerance rides the `Mission` into `plan(with_acceptance=True)`'s `validation` (`as_built_tol_m`/`as_built_pass`), and the compiled keep-outs into the planner's routing + `keepout_conflicts`. `lode/test_mission_intent_compiler.py` [REQ:CP-04] verifies all terms incl. a compiled tolerance failing a tight pad + passing a loose one on a real sloped DEM, and a compiled keep-out routing the real planner around it + flagging a build inside it. I/X/V=D: the tolerances + keep-outs gap from the I-audit is closed; full implementation, integrated, test-verified. | D | D | D | NA |
