...
| TW-04 | P1 | One seeded composite generator combines craters, rocks, material, and illumination parameters. `build_from_dem` currently combines craters (DEM corridor) + material (ChaSTE density) + illumination, but NOT rocks. The rocks-completion (a seeded Golombek boulder layer, commit `bd21352`) was attempted but REVERTED: sampling cm-scale clasts over the demand-refined corridor made `build_from_dem` exceed the 600 s test timeout. `stewie/terrain/test_scenes.py` [REQ:TW-04] validates the current three-family composite + provenance + datum round-trip on the real Haworth DEM. I=P/X=P/V=P (held): next-action = a performance-conscious rocks layer (sample over the coarse backbone grid, not the fine corridor) so all four families combine without the timeout. | P | P | P | NA |
