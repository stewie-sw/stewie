...
| TW-04 | P1 | One seeded composite generator combines craters, rocks, material, and illumination parameters. `build_from_dem` currently combines craters (DEM corridor) + material (ChaSTE density) + illumination, but NOT rocks. The rocks-completion (a seeded Golombek boulder layer, commit `bd21352`) was attempted but REVERTED: the full-suite [REQ:TW-04] test exceeded the 600 s timeout. Diagnosed (run-verified): the cost is `build_from_dem`'s fine-tile corridor refinement (`mosaic.ensure_fine` at `radius_m=20`) which is cold-cache-slow (>90 s even WITHOUT rocks), compounded by the rocks step's surface-snapping over the refined corridor; bounding the boulder count (`sample_boulders d_min_m` 0.04->0.30) does NOT fix it. `stewie/terrain/test_scenes.py` [REQ:TW-04] validates the current three-family composite + provenance + datum round-trip on the real Haworth DEM (the original test, `@skipif` when the DEM bundle is absent). I=P/X=P/V=P (held): next-action = profile + cache/warm `ensure_fine` (or scope the test to a smaller radius / pre-warmed fine tiles) so `build_from_dem` is fast enough to add the rocks family within the timeout. | P | P | P | NA |
