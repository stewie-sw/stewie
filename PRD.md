...
| GW-08 | P0 | **Edit session (= ED-01).** Select/create/modify/delete/measure/snap/undo mission features (keep-outs/waypoints/work-zones) with versioned audit; edits write ONLY through backend routes, never the map layer directly. (extends GL-02) | N | N | N | NA |
...
| RT-04 | P1 | **RViz/Foxglove engineering panel (evidence-only).** Shows topic freshness/TF/robot-model/path/costmap/perception/covariance/diagnostics/SAFE for the selected run; cannot approve/release/command. (extends RS-07/BA-08; needs GW-00) | N | N | N | NA |
...
| EV-01 | P1 | **Evidence/Report reproduces.** A report reproduces plan inputs + selected layers + assumptions + runtime profile + ROS/Gazebo/RViz/Godot artifacts + world transactions + audit trail; a single evidence bundle proves what ran. (extends RF-03/MT-05) | N | N | N | NA |
