...
**Completion snapshot (§7 requirements = 339 — 315 campaign + 24 folded 2026-07-04 from the PRD2 GIS-workbench product target, §7.B; live machine counts in STATUS.json / program_snapshot.json / release_manifest.json).** Raw verification glyphs (2026-07-04): 220 verified-done, 20
...
| AC-02 | P0 | The route registry records per-route pane ownership, auth/role, response kind, provenance requirement, fixtures, and authority-mutation flag; a pane-backed route missing fixture/role/provenance fails. | N  GATED (2026-07-16): `route_registry.py` (the AC-01 coverage registry) is EXPLICIT that AC-02's richer per-route metadata -- pane ownership, auth/role, response kind, provenance requirement, fixtures, authority-mutation flag -- is deferred with the FRONTEND-SHELL decision, because pane ownership presupposes the pane taxonomy (ADR-0007), which is undecided. Building it now would mean inventing a pane taxonomy nobody has chosen -- a design decision that is Aaron's, not a build. Reclassified NA->G (design-gated) rather than forced. | N | N | N | G |
| RF-01 | P0 | React shell implements the same 13 pane identities + role visibility as the vanilla cockpit; signed-in browser tests open all 13 panes at desktop + phone widths. | D | D | D | NA |
| RF-02 | P0 | React workspace state carries mission/site/body/vehicle/physics-backend/product-mode/runnable-profile/source-class/work-area; URL+state round-trip; Release/Execute refuse mismatched profile/backend states. | D | D | D | NA |
| RF-03 | P1 | Each migrated pane ships empty/error/loading/mobile fixtures + a signed-in Playwright parity test vs the vanilla pane before it is flipped. | D | D | D | NA |
| GL-01 | P0 | 2D map workbench renders the selected site DEM + FR-10 layers in the local order frame; control points round-trip /dem/site_xy + /dem/site_lonlat within tolerance; no WGS84/Earth claim on lunar coords. FRAMING: shipped as QWC2/OpenLayers at /ide (QW-01/GW-05), NOT the originally-scoped MapLibre/deck lane, which was never adopted. | N  [REQ:GL-01] VERIFIED (glyph-lag cleared 2026-07-16): the QWC2/OpenLayers /ide (GW-05/QW-01) IS this row's delivery, and its acceptance is proven by the dual-tagged gates in `test_gw05_gis_map_substrate.py` — the /dem/site_xy<->/dem/site_lonlat control-point round-trip within tolerance on real Haworth, and the assertion that the lunar coord is selenographic south-polar, never Earth WGS84. The site DEM renders (GW-05 Playwright-verified on the deployed /ide). The originally-scoped MapLibre/deck lane was never adopted; QWC2 supersedes it. | D | D | D | NA |
| GL-02 | P1 | Map identify/measure/edit sessions operate on the map's vector layers and write mission edits ONLY through backend routes; a keep-out drawn on the map appears in the mission request and routes around it. FRAMING: shipped as QWC2/OpenLayers (not deck); the keep-out->routes-around behavior shipped as GW-08. GLYPH-LAG: the three clauses compose from live routes — (A) identify: GET /world/point resolves a click to the real Haworth DEM cell (out-of-tile→in_bounds=false, no fabricated value); (B) edit-only-through-backend: /edit session store in the map CRS (IAU_2015:30135); (C) keep-out→routes-around: /plan merges the session keep-out, an enclosing circle makes the haul infeasible. [REQ:GL-02] `stewie/server/test_gl02_map_edit_backend_routes.py` (all three end-to-end on real Haworth; no synthetic terrain). | D | D | D | NA |
...
| MG-01 | P0 | The vanilla cockpit at /app stays served + deployable and its smoke tests keep passing. FRAMING/RETIRED: the /app->/app2 React migration this row governs was never adopted -- there is no /app2. Real topology: / (OpenLayers viewer) + /ide (QWC2 workbench = the product front door) + frontend/ (React cockpit, a separate surface). | N  [REQ:MG-01] RESOLVED (2026-07-16): the /app->/app2 React migration this row governs was never adopted (no /app2). `test_mg_frontend_topology.py` asserts the topology of record -- the vanilla cockpit / and the React SPA /app BOTH still serve, both exempt in the route registry -- and the mobile smoke gate [REQ:FR-20] keeps passing. The cockpit stays served; nothing regressed. | D | D | D | NA |
| MG-02 | P0 | RETIRED: the per-pane vanilla->React flip-with-parity-gates this row governs was never adopted (no /app2). The product front door is the QWC2 /ide (b833069); the React frontend/ cockpit is a separate surface, not a pane-by-pane flip of the vanilla cockpit. | N  [REQ:MG-02] RESOLVED-BY-RETIREMENT (2026-07-16): the per-pane vanilla->React flip was consciously dropped (the React rewrite black-screened on Cesium init and was reverted 55c44c6); the product front door is the QWC2 /ide. `test_mg_frontend_topology.py` proves the retirement HELD: no /app2 route exists anywhere in the app or the route registry, and /ide is the artemis-edge workbench (a separate surface), not a pane-by-pane React flip of the vanilla cockpit. The decision is the resolution. | D | D | D | NA |
| MG-03 | P2 | RETIRED: the /app-flips-to-React cockpit-retirement plan this row describes was never adopted (no /app2). Real topology: / OL viewer + /ide QWC2 + frontend/ React. Cockpit consolidation now lives in the ConOps /ide menu (b833069) + the operational-OS reframe (WS-*/TP-*, task #64). | N | N | N | NA |
...
| QW-01 | P0 | The served /ide/ front door is a QWC2 (qwc2-demo-app) SPA whose appConfig registers the STEWIE Mission* plugins over an OpenLayers map; a signed-in browser smoke opens each Mission* plugin at desktop+phone widths with zero blocking console errors. [REQ:QW-01] `stewie/server/test_qw01_ide_appconfig.py` (the python static gate req_trace counts) + `frontend/_ide_qw01_smoke.mjs` (the runtime smoke): appConfig.js imports + registers all 13 STEWIE Mission* plugins (Mission Plan/Program/Layers/UserLayer/Assets/Terrain3D/CrossSection/Terramech/Runtime/Evidence/EngPanel/HUD + SelectionInspector) in pluginsDef over the QWC2 OpenLayers Map plugin, and the signed-in browser smoke (deployed artemis /ide, real GPU) opens EACH via window.qwc2.setCurrentTask at desktop (1600px) AND phone (390px) widths with the OpenLayers map present and ZERO blocking console errors (an external LROC/Trek WMS tile 404 is a non-blocking network warning; an uncaught JS exception would fail). I=D (the /ide front door is the QWC2 SPA served read-only from gis/qwc2/prod per deploy/compose.yml:294; appConfig registers the Mission* plugin suite over the OL Map plugin), X=D (each plugin activates via the qwc2 API task system + mounts its SideBar; the static gate runs in the CI python suite), V=D (the runtime smoke opens all 13 plugins at both widths with 0 blocking errors — pass:true, allActivated:true, zeroBlocking:true — and the python gate asserts the registration + the smoke's two-width zero-blocking shape). [SHIPPED: gis/qwc2/js/appConfig.js, deployed read-only from gis/qwc2/prod, deploy/compose.yml:294] | D | D | D | NA |
...
| PO-19 | P1 | **The §7 plan is a validated ATOMIC TASK GRAPH, and the dispatcher obeys its readiness rule.** The matrix IS the plan, but it was not a graph. (1) **The fan-out was dependency-blind:** `fanout_plan.classify()` bucketed rows by GLYPH and FAMILY only, so its "parallel lanes" were family prefixes, not predecessors -- the opposite of the ATG readiness rule (a node is executable exactly when ALL its predecessors have finished). This was not hypothetical: **RS-05 and RT-03 both declare `needs RT-00`** ("the ROS image carries the stewie python stack"), **RT-00 is NOT done, and both sat in the buildable ready-set** -- the fan-out would have dispatched agents to build the Gazebo rehearsal and the live-sensor loop into a container that cannot import `stewie`, where they fail or fake it. AD-02/AD-03 likewise sat ready while AD-01 was unbuilt. (2) **The order was hand-written prose and it rotted:** the §7.B "Loop pick order" listed 25 rows of which 18 were already done, and every row added since (RT-06, PX-08..PX-11, GW-13) appeared in NO order at all. ATG says the order is DERIVED from the graph. **EDGE SEMANTICS, established from the data rather than assumed** (the PRD already had two words and used them correctly -- RT-03 uses BOTH in one row, `(extends BA-07/RS-05; needs RT-00)`): `needs X` / `Blocks X` / `prerequisite for X` are TRUE prerequisites and therefore ATG EDGES; `extends` / `complements` / `bounds` / `reuses` are LINEAGE and are deliberately NOT edges. Reading `extends` as a dependency would fabricate ~60 phantom edges and falsely accuse seven delivered rows of standing on unbuilt foundations -- the proof it is not an edge is that **GW-07 is V=D (fully tested) while GL-02, which it "extends", is V=N**. New rows may use the canonical `(requires: X, Y)`. An ABSENT edge means "no declared prerequisite" -- the absence of a claim, not a claim of independence -- so coverage is reported (today: 7 edges over 5 of 127 open rows) to keep the gap visible instead of assumed away. Acceptance: the graph is a valid DAG (no dangling edges, no cycles, and no row marked V=D while a row it REQUIRES is unbuilt); the ready-set contains ONLY rows whose prerequisites are all done; a held-back row is reported as BLOCKED and NAMES its blocker (never silently dropped, never silently offered); every §7 row lands in exactly one bucket (done/buildable/gated/blocked/concurrent); and the parallel groups are topological levels DERIVED from the graph. [REQ:PO-19] `scripts/atg.py` + `scripts/test_atg_graph.py` assert all six on the REAL PRD, and the readiness gate is NON-VACUOUS (verified to FAIL when the dispatcher is made dependency-blind again). The ready-set drops 97 -> 93 rows: RS-05, RT-03, AD-02 and AD-03 are now correctly BLOCKED with their blockers named. I=D (atg.py: typed edges, DAG validation, readiness, derived levels; fanout_plan now honours them), X=D (the fan-out dispatcher + the FS-01 assessment gate both consume it), V=D (the six [REQ:PO-19] gates, non-vacuity demonstrated). (governs the fan-out; extends PO-13/FS-01) | D | D | D | NA |
...
| PG-01 | P2 | PostgreSQL/PostGIS as a durable persistence/projection layer, NOT authority: `TwinStore` events mirror to a PostGIS projection; the conserved model still mutates truth. [REQ:PG-01] `stewie/server/test_pg01_world_txn_projection.py` (4 functional tests on the URL-agnostic db.py — the SQLite fallback is the SAME SQLAlchemy code prod runs on Postgres/PostGIS via STEWIE_DATABASE_URL): each committed linked world-state transaction (DT-01 — carrying the TwinStore version+hash + the conserved authority sha + plan + belief) mirrors to a durable `world_txn` projection (queryable by seq/world_sha, holding the full tamper-evident chain), while the in-process TransactionLog journal stays AUTHORITY; the mirror is best-effort (a projection failure is caught + logged, never breaking the authoritative commit) and is wired into world_state_service() ONLY when a durable store is configured (STEWIE_DATABASE_URL set), so CI/dev adds no per-transaction write. I=D (db.py WorldTxnRow + mirror_world_txn/read_world_txns + the WorldStateService projection_sink hook), X=D (wired into world_state_service gated on the DB being configured; tests in the CI python suite), V=D (the projection + non-authority + wiring-gate are verified on the SQLite code path — the same URL-agnostic SQLAlchemy — AND the durable mirror+read are verified LIVE on the real docker-internal PostGIS: inside the rebuilt backend container mirror_world_txn INSERTed + read_world_txns read back a world-txn row on the deploy's Postgres, then the smoke row was deleted (edit_session/edit_audit + the world journal untouched); the app-wired sink IS db.mirror_world_txn, so the SQLite-proven gate composed with the PG-proven projection is the end-to-end. The txn projection is relational — world-txn identities carry no geometry; a PostGIS-GEOMETRY projection of spatial features (footprints/keep-outs) is a separate future extension, not part of the TwinStore-event provenance chain). | D | D | D | NA |
...
| BR-02 | P1 | Mission-snapshot versioning as the branchable replay/ML unit (Moon->Mission 000->001->...->N): a completed mission writes an immutable snapshot to a lineage; a snapshot replays deterministically and is selectable as a branch parent for what-if/retrain (no mission_snapshot producer exists today). (extends BR-01/EG-03) | N  [REQ:BR-02] BUILT + VERIFIED (2026-07-16 fan-out): a completed mission writes an IMMUTABLE snapshot to a Moon->Mission 000->001->N lineage (stewie/twin/mission_snapshot.py) -- the branchable replay/ML unit; test_mission_snapshot.py (re-verified on main, exit 0). | D | D | D | NA |
...
**Loop pick order — DERIVED, not hand-written ([REQ:PO-19]).** Run `python scripts/atg.py`. The order falls out of the dependency graph: level 0 is everything dispatchable NOW in parallel, and a row only descends to a later level if a prerequisite it declares is still unbuilt. The previous hand-maintained list is deleted because it rotted exactly as ATG predicts: it named 25 rows of which 18 were already done, and every row added since (RT-06, PX-08..PX-11, GW-13) appeared in it nowhere. A plan's order is a property of its graph, so it is computed, never transcribed. Declare a prerequisite in the row itself with `needs X` (or `Blocks X` from the other side); `extends` is LINEAGE and is deliberately not a dependency.
...
| RT-00 | P0 | **Prerequisite — ROS image carries the stewie python stack.** `deploy/ros2/Dockerfile.ros2dev` installs pip + the monorepo (numpy/scipy/pydantic/gymnasium + stewie_bodies/dart/lode/forge) so live-spine rclpy nodes import `run_replay`; a container test imports `stewie.runtime.run_replay` and runs it headless on a real DEM slice. Blocks RS-05, RT-03. | N  [REQ:RT-00] BUILT + VERIFIED IN-CONTAINER (2026-07-16): `Dockerfile.ros2dev` now installs the stewie python stack -- the pinned scipy/pydantic/gymnasium + the monorepo (stewie/dart/lode/leap/forge + packages/stewie-bodies/forge) -- into the container's SYSTEM python (the interpreter rclpy uses), so a live-spine node imports `stewie.runtime.replay_loop.run_replay`. NUMPY IS ROS'S (debian 1.26.4, which rclpy is built against): forcing the host's 2.4.6 both fails (cannot uninstall a debian numpy) and would risk breaking rclpy, so the stack installs against ROS's numpy and a BUILD-TIME import gate proves the compat -- `[RT-00] run_replay import OK` fired in the build. Verified: image builds (BUILD_EXIT=0), `docker run` shows the stewie_* ROS packages + run_replay importable, and `test_rt00_ros_python_stack.py` asserts the Dockerfile carries the pinned install + gate AND runs run_replay headless on a real Haworth DEM slice (the exact operation a live node performs), with an opt-in in-container check (STEWIE_RUN_DOCKER_TESTS=1). Unblocks RS-05, RT-03. | D | D | D | NA |
| GW-05 | P0 | **GIS map substrate.** The QWC2/OpenLayers map in the LOCAL south-polar-stereographic frame (IAU_2015:30135) renders NASA Trek raster tiles + local LOLA terrain-RGB (heightmap.rf32, full 5 m native, not a downscaled preview); control points round-trip /dem/site_xy + /dem/site_lonlat within tolerance; no WGS84/Earth claim on lunar coords. [REQ:GW-05] `stewie/server/test_gw05_gis_map_substrate.py` (the python gate req_trace counts) + `frontend/_ide_gw05_terrain.mjs` (the live render proof): the QWC2/OpenLayers /ide map renders in IAU_2015:30135 (defaultMapCrs) with a selenographic lon/lat readout (30100), draping the local LOLA terrain — the REAL full-resolution Haworth DEM COG (data/gis/cog/Haworth_1m_dem.tif + per-site DEM/hillshade/slope COGs, NOT a downscaled preview) via qgis-server (QG-02) over the NASA Trek/LROC WAC+NAC south-pole rasters; a control point round-trips /dem/site_lonlat↔/dem/site_xy sub-metre on the real Haworth DEM; the coordinates are selenographic south-polar (lat<-80°, MOON_ME R=1737400 m), never Earth WGS84. I=D (the /ide app substrate QW-01 + the qgis-server .qgz terrain QG-02, both D, realize the map; the map + display CRS are the lunar frames), X=D (the /ide loads the qgis-server /ows GetMap terrain tiles + the LROC Trek rasters; the round-trip transforms are the site_dem inverses, live on /dem/site_xy + /dem/site_lonlat), V=D (the [REQ:GW-05] gate asserts the sub-metre round-trip + the lunar CRS + the real full-res COG; the deployed /ide is Playwright-verified — the LOLA south-pole hillshade + the per-site terrain-RGB drapes render in 30135, 2 /ows GetMap tiles HTTP 200, 0 console errors, coordinate readout selenographic). (realized at /ide by QW-01 + QG-02; the MapLibre lane was never adopted) | D | D | D | NA |
...
| GW-02 | P0 | **Unified workspace context.** One routeable context (the PRD2 field set: mission/site/body/CRS/local-frame/vehicle/fleet/selection/layers/mode/profile/source/namespace/backend/depth/time/branch/release/run/role) drives map/assets/Godot/RViz/Gazebo-evidence/report/authority; one URL restores all of it. (extends RF-02) | D | D | D | NA |
| GW-03 | P0 | **Layer-eligibility UI.** [REQ:GW-03] The layer manifest UI differentiates display/planning/release/execute eligibility + exposes freshness/provenance/uncertainty; a display-only layer cannot be marked planning-valid. (extends DW-01; binds LY-01) | D | D | D | NA |
| GW-04 | P1 | **Asset Library.** [REQ:GW-04] Browse/search/inspect/export/recover durable assets (missions/sites/DEMs/layer-packages/vehicles/plans/rehearsals/runs/reports/evidence/bags/captures), separate from visible layers; every durable object traces to provenance. | D | D | D | NA |
| LY-01 | P0 | **Layer catalog/registry (backend).** The ~65 named layers (`base.*`/`terrain.*`/`hazard.*`/`traffic.*`/`regolith.*`/`physics.*`/`mission.*`/`design.*`/`map.*`/`robot.*`/`runtime.*`/`evidence.*`) each declaring type, body/site/frame, source_class, freshness, uncertainty, provenance, planning/release/execute eligibility, and export formats; an imported ArcGIS/OGC/GeoJSON/COG layer passes CRS/frame validation before it can influence planning. Acceptance fixture = the PRD2 catalog table (design doc L204-278). (extends the `/world` layer_manifest) | D | D | D | NA |
| LY-02 | P1 | **Layer-consumption inspector.** For a selected layer, show where it is consumed: display / planner / costmap / rehearsal / release / execute / report / export. | D | D | D | NA |
| PH-01 | P0 | **Physics backend registry.** Exposes per backend (tier2_numpy/gazebo/chrono/hardware/godot) the authority scope (terrain/robot/sensor/contact/rendering/hardware), mass-conservation, calibration id, per-profile validity (planning/rehearsal/release/execute), refusal reasons, and evidence artifacts. (extends PX-02) | D | D | D | NA |
| PH-02 | P1 | **Physics attribution.** The planner + reports attribute every route/volume/risk value to a physics backend + calibration source; a value with no backend attribution is not release-eligible. | D | D | D | NA |
| TM-02 | P0 | **Terramechanics spine (inspectable terms).** Slope/roughness/regolith/bearing/sinkage/slip/traction-margin/excavation-resistance/energy/uncertainty are exposed as inspectable terms, each classified simulated/sensed/estimated/learned/assumed; a route cost exposes its terms, not only a scalar. (complements the TM-01 calibration gate) | D | D | D | NA |
| TM-03 | P1 | **Terramechanics output layers.** Terramechanics generates traversability/energy/slip-risk/bearing-sinkage/excavation-feasibility/constructability layers into the LY-01 catalog with source-class + uncertainty. | D | D | D | NA |
| TM-04 | P1 | **Predicted-vs-observed terramechanics.** A rehearsal/report compares predicted slip/sinkage/energy against observed or replayed telemetry when available; as-built excavation updates terrain memory + future cost layers. | D | D | D | NA |
| TR-01 | P1 | **Frozen scenarios: a NAMED, pinned, content-hashed section of a real site — the precondition for training on recorded runs.** A demonstration is only training data if the world it was recorded in IS the world you later train and evaluate in. viz2's world was already deterministic (real DEM + deterministic flattest-interior spawn + seeded Golombek rock draw), but two things were missing and both are quietly fatal to a dataset: (1) **you could not NAME or CHOOSE a section** -- the spawn was implicit, so pinning a slice meant remembering raw IAU_2015:30135 metres; (2) **nothing PROVED the world was unchanged** -- with no fingerprint, a change to the terrain window, the spawn search, or the rock draw would SILENTLY invalidate every run ever recorded (still replayable, still plausible, and wrong). Silent invalidation is the worst failure a dataset can have, because nothing tells you. Also repairs a DEAD KNOB: the rock field was seeded `world_seed=0` HARDCODED in the runtime while the stream's `world_seed` config only ever reached the PROCEDURAL/synthetic bundle path -- so on a real site the seed did nothing, and rock-layout domain randomisation was impossible. Acceptance: a scenario is an immutable declaration (site + pinned section + resolution + drum + rock seed); the same name yields a BYTE-IDENTICAL world (terrain, spawn, grid AND every rock); the pinned section is actually honoured (no silent fallback to the auto-spawn); the rock seed is LIVE (changing it changes the world) while its default reproduces the pre-change world exactly (so nothing already recorded is moved out from under); and a PINNED FINGERPRINT of the world FAILS LOUDLY on any drift. [REQ:TR-01] `stewie/dataset/scenario.py` + `stewie/dataset/test_scenario.py` assert all six on the REAL Haworth SfS bundle. The fingerprint hashes what the scenario PRODUCES (real terrain heights in the active window + resolved spawn + grid + every clast's position/size), never its own declaration -- hashing the declaration would be circular and would agree with itself no matter what the code did. NON-VACUOUS: a 1-metre shift of the section changes the hash and is caught. First frozen scenario: `haworth_pad_a` (haworth_sfs_2km_1m @ -34746, 89673, 5 cm, large drum, rock_seed 0), fingerprint `8e48b858ae9ccaa1...`. When this guard fires after a DELIBERATE world change, the correct response is to re-record or re-label the dataset -- not to quietly update the constant. I=D (Scenario + registry + fingerprint; rock_seed wired through the runtime), X=D (the runtime honours a pinned section and a declared seed; default preserves the existing world), V=D (the six [REQ:TR-01] gates on real terrain, non-vacuity demonstrated). (the precondition for the record->train pipeline; composes with PX-09/PX-10/PX-11) | D | D | D | NA |
| TR-03 | P0 | **The OPERATOR selects the worksite section from the /ide map — the rover spawns where you point, not where the terrain is flattest.** REPORTED AS A RENDER BUG ("why doesn't the actual dem load in Godot? the terrain is flat"), and **nothing was broken in the render**: the DEM loads, the height texture is `Image.FORMAT_RF` (32-bit float, not 8-bit), `viz2_window.gdshader` displaces (`VERTEX.y += h`), and the far-field mesh renders. **The picture was honest — the terrain really IS flat where the rover spawns.** Measured on the real bundle: the fine window (240×240 @ 5 cm = **12 m × 12 m**) has **0.083 m of relief and a 0.28° mean slope**, and the raw DEM over that same 12 m footprint agrees (**0.060 m**) — the render is faithful to the centimetre. TWO DESIGN CHOICES CAUSED IT: (1) `viz2_runtime._flattest_interior_spawn` ([REQ:AS-15]) — a legitimate fix (the tile's blind geometric centre can land on a crater wall and entrap/tip the rover on arrival) that hardened into **the only behaviour**, and it does not merely find flat ground, it finds **the flattest ground in the ENTIRE TILE**, so every session opened on the most boring 12 m in Haworth; (2) the render window is only 12 m across — there is no landscape inside a 12 m box on a crater floor. Real relief exists and the spawn avoids it: ±25 m → 1.36 m, ±100 m → 11.91 m, ±500 m → 34.82 m. **AND THE CAPABILITY WAS ALREADY THERE.** `Viz2Runtime(start_xy=…)` is a real constructor parameter (it only calls the flattest search when `start_xy is None`), and `viz2_serve.py` has always exposed `--start-xy`. The chain broke in exactly **two** places — `protocol.parse_config` had no `start_xy` field and `app.py` never passed `--start-xy` — so **the browser could not even ASK**. Same defect shape as the cosmetic arm (PX-10), the dead rock seed (TR-01) and the modelled-but-unused dig reaction (PX-13): a real knob with nothing connected to it. **NO REPROJECTION ANYWHERE:** `gis_layers.py:1208` confirms the public /ide map is already in **IAU_2015:30135 metres** ("a 30135 click reprojected to 30100…"), so a click on it **IS** `start_xy` in the frame the runtime wants. Acceptance: the wire protocol carries the chosen section; an unchosen spawn stays `None` so the **safe flattest fallback still runs** (no regression — you cannot drop into an entrapment); the stream server actually forwards it (`build_runtime_argv`, extracted as a pure function precisely so the plumbing is ASSERTABLE and cannot rot silently again); a click on the /ide map ARMS a section and an explicit **⌖ Drive here** applies it (arming is separate from driving because a spawn defines the WORLD — applying it opens a new session; you cannot teleport a conserved world); and the chosen section is the section you actually drive. **PUBLIC-SURFACE DISCIPLINE (RT-06/RT-07):** an anonymous browser sends this coordinate, so it is UNTRUSTED — shape and finiteness are REFUSED at ingest (never coerced, never defaulted, exactly like the twist M-04 and the arm angle PX-10; a NaN spawn would place the rover nowhere and poison every downstream index), and an out-of-tile coordinate is **REFUSED, never clamped** to an edge — a clamp would drop the rover somewhere the operator never chose *while looking like it worked*, which is the worse failure. The bounds check deliberately lives in `app.py`, not `protocol.py`, because that module is **validation-only and does NO I/O** by contract and the tile extent lives in the bundle metadata; `protocol` owns shape+finiteness (pure `start_xy_in_bounds` predicate), `app.py` owns bounds. [REQ:TR-03] `stewie/stream/test_operator_spawn.py` (15 gates incl. the parametrized malformed-input set). Real TDD (RED first). **ANTI-DECORATION GATE — the lesson of this whole cluster, applied pre-emptively:** a `start_xy` that plumbs correctly but lands you on the same flat pad would be worthless, so a gate asserts the chosen section carries **>10× the relief** of the flattest-search default. Measured, same tile, same DEM, no code change: `haworth_pad_a` **0.06 m / 0.28°** vs `(-33450, 88788)` **3.99 m / 13.87°** — **66× the relief**, and **544,377** such windows exist (6–14°, inside IPEx's 15° nominal envelope). I=D, X=D (live on the public /ide), V=D. (needs RT-06; composes with TR-01 — a NAMED frozen scenario and a MAP-CHOSEN section are the same capability from two ends; unblocks TR-02) | D | D | D | NA |
| TR-02 | P1 | **Interpretable search-distilled excavation/construction policy — a decision tree over PHYSICS, auto-researched against the exact conserved sim.** The oracle is MODEL-BASED SEARCH (beam/CEM) run in the exact, mass-conserving, sub-millisecond authority on a frozen scenario (TR-01), which yields optimal maneuver labels WITHOUT needing human demonstrations. Those labels are distilled into a **depth-limited decision tree / rule list** over features that are already real, named telemetry — `avg_slope_deg`, `drum_fill_frac`, `dig_j_per_kg` (FEE, ∝ depth²), `dig_reaction_n`, `dig_engagement`, `max_cut_per_pass_m`, `inventory_kg`, `odometry_error_m` — plus per-cell state from the conserved authority (`derive_height`, `diff_field` = as-built vs virgin, `cell_strength`, `bearing`, `wheel_static_sinkage`, slip). Because the features ARE the physics, a split MEANS something (`slope > 12° AND drum_fill > 0.8 → haul before digging`), and the resulting policy is a **readable operating procedure you can defend to a review board** — which for a NASA-facing artifact is the deliverable, not a nicety. WHY A TREE AND NOT A NET: this project's own evidence says model-based search + distillation is the lever, not the network class — **beam search 24 legs > PPO 27 > greedy 28, and the search-distilled MLP hit the optimum (24)**. A tree is simply a different, INTERPRETABLE distillation target for the same proven oracle. **ACCEPTANCE, with the null result pre-committed:** the tree must BEAT GREEDY on passes-to-spec / energy-to-spec / as-built RMSE (±2 cm), and be scored against the beam optimum. **A tie with greedy is a NULL RESULT and MUST be reported as one** — the prior finding that greedy AND RANDOM both hit 100% on the unconstrained flatten is exactly the trap this row exists to avoid; RL/search only earns its keep under real constraints. Generalization is tested on **HELD-OUT rock seeds** (train seeds 0..N, evaluate unseen draws — same terrain, different clast field), which is possible ONLY because TR-01 made `rock_seed` live. Trees are axis-aligned, so ENGINEER THE PHYSICS INTO THE FEATURE (hand it `predicted_fee_j_per_kg(candidate_bite)` rather than making it rediscover energy ∝ depth²) — using the exact model we already own is the point, not a shortcut. PRIOR ART, stated so this row does not overclaim: **NTRS 20210022218** (ICE-RASSOR *learning excavation*) is NASA replacing the AutoDig PID trencher with deep RL — so the contribution here is NOT "apply learning to digging"; it is (a) an INTERPRETABLE policy instead of a black box and (b) model-based distillation, which this repo's own numbers say beats model-free. NOTE: this path does NOT need human demonstrations — the oracle is the search — so it is NOT gated on the observation-paired recorder (that remains necessary only for distilling OPERATOR intuition, where the cost function is wrong: slumping, rock avoidance, approach angle). **MEASURED RESULT — NULL (2026-07-16), and reported as one per this row's own pre-commit.** On the AS-IMPLEMENTED excavation physics the distilled tree TIES GREEDY, and it does so because all three acceptance metrics are policy-INVARIANT, not because the search was weak. `ipex_specs.dig_energy_per_kg()` is a **CONSTANT** J/kg (ipex_specs.py:167 = P_dig/dig-rate; "BP-1-calibrated, material/density/ice-independent", EP-02) with **no depth term** — the row's assumed `dig_j_per_kg (FEE ∝ depth²)` is NOT in the code — so with mass conserved the **energy-to-spec (= const·cut_mass)** and **passes-to-spec (= ceil(cut_mass/capacity))** are TARGET-determined, and `WorkSite.flatten` is deterministic + mass-exact so the **as-built RMSE** is fixed by the target, not the policy. MEASURED on the real Haworth authority + real Golombek rock field, held out by `rock_seed`: four distinct policies (greedy-highest, greedy-nearest, **RANDOM** — the exact control the row names — and shallow-first) tie to float-epsilon on passes/energy/RMSE across seeds 0–5. The ONE policy-sensitive term is haul-leg drive energy, dig-dominated to **<0.3%** and itself the SchedulerEnv leg-count problem where this repo already found greedy=beam=optimal. **UNLOCK (so the null is actionable, not a dead end):** implement a SOURCED depth-dependent excavation-force model (real FEE — Reece/Balovnev/McKyes draft ∝ cut depth) so bite depth becomes a genuine energy lever — a FORGE physics row, NOT a distillation; inventing the depth² term here would fabricate data (forbidden) — OR retarget the interpretable policy to the MULTI-VEHICLE scheduling layer (parallelism + space-time conflict), where this repo's own M4 note records real headroom. [REQ:TR-02] `lode/test_tr02_excavation_policy.py` (5 gates: depth-independent-constant root cause, real held-out seeds, the cut meets ±2 cm + removes real mass, the four-policy tie per seed, dig-dominance) + `lode/tr02_excavation_policy.py`. I=D (experiment + policies + the three metrics on the real conserved authority), X=NA (there is NO beating-greedy policy to expose — that is the null), V=D (the null is verified + regression-gated). (needs PX-14, TR-01 — both done) | D | NA | D | NA |
| SD-01 | P1 | **Surface Design view.** [REQ:SD-01] Pads/berms/trenches/cuts/fills/roads/stockpiles/sinter → typed mission orders with volume/mass/bearing/sinkage/constructability evidence; volume/mass estimates show DEM resolution, material assumption, and uncertainty; a before/after terrain transaction influences the next plan. | D | D | D | NA |
| RT-01 | P0 | **Runtime profile registry.** A GET endpoint defines desktop_sil/digital_twin/ros2_replay/gazebo_sim/hil/field_test/live_rover and each profile's allowed command + evidence capabilities; the cockpit binds it. (extends PX-02/FR-01; names gazebo_sim) | D | D | D | NA |
| RT-02 | P0 | **Evidence bound to run/profile.** ROS/Gazebo/RViz/Godot evidence surfaces attach to the selected run/profile and hold no independent command authority; the execution service is the sole command egress. (extends BA-08) | N  [REQ:RT-02] VERIFIED (glyph-lag cleared 2026-07-16): `test_ros_evidence.py` proves the ROS/Gazebo/RViz evidence surface is structurally read-only (its AST names no release/execute/advance/cmd_vel/rclpy path, no POST), is bound to the committed AS-01 autonomy contract's node graph (not fabricated), and that the command verbs live ONLY on the director-gated executive router -- a different module the evidence surface does not import. The execution service is the sole command egress; an evidence pane cannot command the rover. | D | D | D | NA |
| RT-03 | P1 | **Gazebo rehearsal on real DEM.** The gazebo_sim profile launches the selected site's REAL DEM heightfield (not a flat plane) + truth-isolated sensor topics; the cockpit shows `/clock`, bridge freshness, bag path, and the truth-denial assertion; a rehearsal returns predicted path + sensor evidence + divergence. (extends BA-07/RS-05; needs RT-00) | N | N | N | NA |
| RT-04 | P1 | **RViz/Foxglove engineering panel (evidence-only).** Shows topic freshness/TF/robot-model/path/costmap/perception/covariance/diagnostics/SAFE for the selected run; cannot approve/release/command. (extends RS-07/BA-08; needs GW-00) | D | D | P | G |
| RT-05 | P1 | **Godot mission view (sidecar-first).** Renders the selected branch/run (terrain/rover/path/hazards/sensor-cones/shadow/excavation/as-built) using the same site/body/frame as the map; captures carry run/branch id, camera pose, timestamp, source class, asset hashes; never physics or command authority. (extends BA-08) | N | N | N | NA |
| RT-06 | P1 | **Drive-validate mission view embedded in the /ide (viz2) — SIM-scoped command authority.** The QWC2 IDE exposes `Validate ▸ Drive 3D`: a SideBar panel embedding the viz2 Godot pixel-stream (host-GPU, xvfb+Vulkan, the REAL Haworth SfS 1 m DEM in IAU_2015:30135) as an iframe, and FORWARDING the mission route authored on the 2D map into it (`postMessage {type:'stewie:plan',waypoints,lonlat}` -> viz2 `_plan_goals` -> `lode.route_leg` slope-costmap A*) so the operator DRIVES the exact authored plan on the real surface rather than a look-alike. This is the drive-validate surface; it is NOT RT-05 (which remains the read-only capture/evidence sidecar and stays unbuilt). **Command scope is the load-bearing constraint:** unlike an evidence panel, this surface HAS command verbs ({cmd_vel}/{safe}/{rearm} + the excavation actuators), and it ships on the PUBLIC front door -- so §7.B's "never an independent command surface" is honored in its safety intent by making the scope SIM-ONLY and TESTING it: actuation targets the in-process conserved plant + Godot, and the stream server holds ZERO real-rover egress (no rclpy/live node, no executive/release path), preserving RT-02 sole-egress and the AG-08 real-instruction gate. `{cmd_vel}` still LOWERS through the real `ros2_bridge.twist_to_command` + `rc_contract.Pose` (a genuine shape/conformance proof, not a mock), and a contract `{safe}` is an ALL-STOP (refuses motion AND dig/dump/drum/arm until `{rearm}`), not a wheels-only stop. [REQ:RT-06] `stewie/stream/test_viz2_ide_embed.py` asserts all three against the REAL `pump_input` handler (GPU isolated: no Godot spawned, the Godot TCP seam replaced by a capturing writer, so the latch/gate logic under test stays real): (a) an AST guard proves the publicly reachable stream server imports/constructs NO real-rover egress; (b) `{cmd_vel}` is acked with the real lowered contract kind and the twist reaches only the sim seam; (c) a `{safe}` latch pushes an immediate all-stop frame then refuses cmd_vel AND every excavation actuator, restoring on `{rearm}`; (d) MissionDrive3D is registered in BOTH QWC2 places + carries the Validate menu entry (the python-visible gate for a JS plugin, since req_trace scans python only). All four are NON-VACUOUS -- verified to FAIL on an injected `make_live_node` import, on dropping `dig` from the SF-01 refuse-set, and on removing the menu entry. I=D (plugin `gis/qwc2/js/plugins/MissionDrive3D.jsx` + the viz2 plan-ingest/postMessage seam + the ROS console + the SF-01 latch, all live), X=D (registered in appConfig.js + config.json, DEPLOYED to the public front door: artemis.stewie.space/ide/ embeds the cross-origin viz2.stewie.space stream; viz2 runs as a durable systemd --user unit behind the cloudflared ingress), V=D (the [REQ:RT-06] gates above + a real Playwright run THROUGH Cloudflare: Validate▸Drive 3D opens the panel, the cross-origin stream renders the real Haworth surface, and a forwarded lon/lat route drove it -- 121.1 m / 3 wp -- with zero console errors). (complements RT-05; scoped by RT-02/AG-08/SF-02; deploy runbook in `deploy/DEPLOY.md`) | D | D | D | NA |
| RT-07 | P0 | **viz2/Godot DECLARES a runtime profile (`godot_sim`), and the declaration is PROVEN rather than promised.** The RT-01 registry enumerates the execution environments a mission may run in and each one's command + evidence authority, and its whole purpose is the fail-closed guarantee that *a simulation can never command real hardware*. It listed `gazebo_sim` — "drives the SIM rover under truth-isolated sim_truth, never the real rover" — a sentence that describes **viz2 exactly**. But **viz2 was not in the registry at all**: neither `viz2_runtime` nor the stream server declared any profile. So the ONE simulator that is actually built, driveable, and **reachable from the public internet** (RT-06) sat entirely **outside** the authority model meant to govern it. That is a governance hole on a live public surface, and it is what this row closes: `godot_sim` is the profile viz2 has always *been*; it simply had no name. **AND THE PART THAT MAKES IT REAL:** the registry is **DECLARATIVE — nothing enforces it.** Outside `runtime_profiles.py` and its own test, EVERY mention of `can_release`/`can_execute` in the codebase is inside a **docstring** (`routers/models.py:226/233/234`, `routers/evidence.py:126`). A profile entry on its own is a comment with a type annotation, and shipping one as "safety" would be precisely the bug class this project keeps finding: the cosmetic arm (PX-10), the dead rock seed (TR-01), the modelled-but-unused dig reaction (PX-13). **A flag nothing reads is not a control.** So the declaration is WELDED to the structural proof that already exists: viz2's real safety is not a boolean but RT-06's AST egress guard, which proves the public stream server cannot even *name* a real-rover command path. Acceptance: the registry gains `godot_sim` (kind simulation, command `bounded`, evidence `sim_truth`, `can_release`/`can_execute` BOTH False); the stream server DECLARES it (`RUNTIME_PROFILE = "godot_sim"`, on the runtime, not in a doc); the declared profile's no-command-authority claim is **asserted together with the AST proof** that the module is structurally incapable of exercising any — so if an egress is ever added, the gate fails *even though the dict still says False*, which is the entire difference between a claim and a control; the escalation ladder (none→bounded→full) still holds; and **the Godot/Gazebo split is recorded as a test** so nobody collapses one into the other. [REQ:RT-07] `stewie/stream/test_rt07_godot_profile.py` (6 gates) + the extended [REQ:RT-01] registry gate. Real TDD (all 6 RED first). NON-VACUOUS: falsely declaring `live_rover` makes the weld FAIL (verified — 2 gates go red). **ARCHITECTURAL DECISION OF RECORD (the question that prompted this row: "isn't Gazebo what we're doing in Godot?"): they are NOT redundant — same authority, different jobs.** `gazebo_sim` is the **ROS2-native robot/sensor** surface (URDF→SDF via `stewie.interop.xacro_to_sdf`, `ros2_control`, sensor plugins on real ROS topics) — the integration path for Nav2/SLAM/**Autoware** perception, which viz2 CANNOT serve because it speaks a private loopback-TCP JSON protocol, not ROS topics. `godot_sim` owns the **conserved terrain + excavation authority** (mass-exact cut/fill — **Gazebo has no mass-conserving deformable terrain, which is exactly why the numpy authority exists**) plus the lunar photometry. Delete either and a capability the other does not have goes with it. **OPEN RISK, NAMED NOT CLOSED:** if perception later runs in Gazebo while excavation runs in Godot, the two could diverge into **two terrains and two rover dynamics**, and evidence from one would not transfer to the other. The rule that prevents it — *Gazebo must NOT own terrain; it consumes the conserved authority's heightmap (the Seam-1 state-field contract), one truth two engines* — is **stated here but NOT yet verified in the container**; that verification is its own row. I=D (profile + declaration), X=D (the publicly-reachable sim is now inside the authority model), V=D (the 6 [REQ:RT-07] gates + the weld + non-vacuity). (extends RT-01; enforced by RT-06's egress guard; prerequisite for nothing — it is a governance fix on a live surface) | D | D | D | NA |
| AU-01 | P0 | **Global command-authority card.** Visible from every command-capable view whenever command capability exists, showing every refusal reason; a disabled control names why. (extends FR-03) | D | D | D | NA |
| EV-01 | P1 | **Evidence/Report reproduces.** A report reproduces plan inputs + selected layers + assumptions + runtime profile + ROS/Gazebo/RViz/Godot artifacts + world transactions + audit trail; a single evidence bundle proves what ran. (extends RF-03/MT-05) | D  [REQ:EV-01] VERIFIED (V=P->D, 2026-07-16 fan-out): the last unmet axis, ASSUMPTIONS, is now reproduced from COMMITTED sources only (conserved density profile + body gravity from stewie.specs.constants, the terramechanics spine's live per-term calibration status, and the tier2_numpy physics authority) and folded into the deterministic bundle_sha; all 6 axes reproduce. test_ev01_evidence_bundle.py (re-verified on main, exit 0). | D | D | D | NA |
| QG-02 | P1 | QGIS-Server publishes the shared-core .qgz as OGC WMS/WMTS/WFS (one project, two clients): a --profile gis GetMap on a site in IAU_2015:30135 returns a pole-truthful tile matching QGIS Desktop. [REQ:QG-02] `stewie/server/test_qg02_qgis_server_wms.py` (the python gate req_trace counts) asserts the one-project-two-clients contract at rest: gis/stewie_south_pole.qgz (the SAME .qgz QGIS Desktop opens) declares the pole CRS IAU_2015:30135 (269 refs, the project working frame) + the STEWIE layer set (36 layers incl Haworth DEM/Hillshade + the Artemis Site layers the GetMap LAYERS= names), and deploy/compose.yml wires the qgis-server service (image qgis/qgis-server:3.34 for PROJ 9.4 / the IAU registry, opt-in 'gis' profile, the .qgz mounted at the MAP= path). RUNTIME evidence (the acceptance's live GetMap): verified 2026-07-10 against the :8082 container — `--profile gis` WMS GetMap Site01 in IAU_2015:30135 returned a real 1400×1400 non-blank pole-truthful relief tile (16709 unique colors), byte-identical to the QGIS Desktop proof render (gate P1.8, gis/SERVER.md). I=D (the .qgz + the qgis-server compose service publish the shared-core project as OGC WMS/WMTS/WFS), X=D (the same .qgz is the QGIS Desktop + QGIS-Server surface; the gate runs in the CI python suite, the GetMap is opt-in local per the 'gis' profile), V=D (the [REQ:QG-02] gate asserts the .qgz CRS + layers + the compose service, and a live GetMap renders a non-blank pole tile matching Desktop). [SHIPPED: gis/stewie_south_pole.qgz + deploy/compose.yml:251 P1.8] | D | D | D | NA |
| QG-03 | P1 | QGIS Desktop authoring/analysis workbench over the SAME FastAPI/PostGIS backend as the web IDE: thematic .qgz templates open the STEWIE layer catalog from the backend; a Model Builder model chains >=3 STEWIE Processing algorithms and reruns headless via qgis_process with identical params; selecting a mission Analysis Profile filters the toolbox to that profile's algorithms. | N | N | N | NA |
| QG-01 | P2 | A STEWIE QGIS Processing provider registers the STEWIE analyses (StewieTerramechanics/StewieSamplePoint) as Processing algorithms runnable from the QGIS GUI, qgis_process CLI, batch, and Models over the public backend; the QGIS-free stewie_backend fetch+parse is unit-tested in CI with no qgis.* in the test path. [REQ:QG-01] `stewie_qgis/test_stewie_backend.py` (in the CI testpaths per pyproject.toml, runs WITHOUT a qgis runtime): the QGIS-free stewie_backend fetch+parse is unit-tested (url builders encode the site, terramechanics-spine + point-attribute normalization, honest ValueError on !ok) against REAL backend curl fixtures, and — by reading the provider source, still qgis-free — StewieProvider.loadAlgorithms registers BOTH StewieTerramechanics + StewieSamplePoint as QgsProcessingAlgorithms. I=D (stewie_qgis/: stewie_backend.py the QGIS-free fetch+parse, stewie_provider.py registers the 2 algorithms, stewie_algorithms.py the Processing algorithms runnable from GUI/qgis_process/batch/Models), X=D (the provider + algorithms run on the host via a headless pyQGIS smoke; the qgis-free backend is in the CI python testpaths), V=D (the [REQ:QG-01] test proves the fetch+parse on real fixtures + the two-algorithm registration, with no qgis.* in the CI test path). [SHIPPED: stewie_qgis/, test_stewie_backend.py] | D | D | D | NA |
| GW-10 | P1 | A per-component monotonic request guard drops a stale in-flight response so a slow site-A load resolving after a switch to site-B cannot overwrite B's raster/physics/inspector state. (hardens GW-02) [REQ:GW-10] `stewie/server/test_gw10_request_guard.py` (the python static gate req_trace counts) + `gis/qwc2/js/mission/reqGuard.test.js` (the node runtime proof, 4/4): the guard is a monotonic token guard (next issues a rising token; current(tok) is true only for the latest; bump/next invalidate every in-flight token), the wrong-site race is dropped (a slow site-A load resolving after a switch to site-B returns current=false → not applied), and it is HELD per-component by the three surfaces GW-10 names — the raster (MissionTerrain3D.jsx), the physics/profile (MissionCrossSection.jsx), and the inspector (SelectionInspector.jsx), each with its OWN makeReqGuard()/next()/current-or-bump (seq local to the factory, no shared singleton). I=D (gis/qwc2/js/mission/reqGuard.js makeReqGuard exposes next/current/bump with a per-component seq; hardens GW-02), X=D (wired into the raster/physics/inspector loads + planAuthor's plan/bbox fetch, all dropping a stale prior-site resolve; the node test runs in the CI JS tier, the static gate in the python suite), V=D (the node test proves the wrong-site race + per-component isolation at runtime; the python static gate asserts the guard shape + the three-surface wiring, so req_trace's python scan counts it). [SHIPPED: gis/qwc2/js/mission/reqGuard.js + reqGuard.test.js] | D | D | D | NA |
| LY-03 | P1 | User-created/imported layers with an Earth-CRS reject gate: an imported GeoJSON declaring an Earth CRS (EPSG:4326/3857/CRS84/WGS84) is rejected with a legible reason before it touches the map or is promoted planning-eligible; a valid lunar-frame (IAU_2015:30135/30100) layer is accepted. (extends LY-01) [REQ:LY-03] `stewie/server/test_ly03_user_layer_crs_gate.py` (the python static gate req_trace counts) + `gis/qwc2/js/mission/userLayers.test.js` (the node runtime proof, 4/4): validateLayerCrs REJECTS the Earth CRSs (EPSG:4326/3857/CRS84/WGS84 + 2056/generic-EPSG) with a legible reason pointing at the lunar frame and ACCEPTS the lunar frame (IAU_2015:30135/30100); MissionUserLayer.jsx runs parseUserLayer + validateLayerCrs and RETURNS on a failed validation BEFORE map.addLayer, so an Earth-CRS import can never misplace features on the lunar map or be promoted planning-eligible. I=D (userLayers.js validateLayerCrs: Earth-reject / lunar-accept / no-crs-soft-note / unknown-reject, pure + node-testable), X=D (wired into MissionUserLayer.jsx's import flow — the gate blocks before the OL vector layer + the promote-to-planning path; the node test runs in the CI JS tier, the static gate in the python suite), V=D (the node test proves the reject/accept at runtime; the python static gate asserts the reject/accept branches + the block-before-map wiring, so req_trace's python scan counts it). [SHIPPED: gis/qwc2/js/mission/userLayers.js + userLayers.test.js] | D | D | D | NA |
| SD-02 | P1 | Cut/fill material-balance: for cut+fill orders materialBalance returns bank cut volume, loose spoil (RHO_DEEP->RHO_SPOIL bulking), compacted fill demand, and net borrow/spoil in kg with mass conserved (cut-only => net spoil, fill-only => net borrow). (extends SD-01) [REQ:SD-02] `stewie/server/test_sd02_material_balance.py` (the python static gate req_trace counts) + `gis/qwc2/js/mission/planTools.test.js` (the node runtime proof, 24/24): materialBalance conserves MASS — cut excavated at bank density (RHO_DEEP=1920), placed spoil bulks to loose (RHO_SPOIL=1300, +~48% volume) — returning bank cut/fill volumes, loose spoil, cut/fill mass, and the net balance_kg with the surplus/deficit direction (cut-only ⇒ net spoil surplus; fill-only ⇒ net borrow deficit, both asserted). I=D (planTools.js materialBalance: conserved-mass balance + RHO_DEEP/RHO_SPOIL bulking + net direction, exported for the mission panel), X=D (surfaced in the plan-tools panel; the node test runs in the CI JS tier, the static gate in the python suite), V=D (the node test proves the swell + surplus/deficit + cut-only/fill-only net cases at runtime; the python gate asserts the conserved-mass formula + the moon-density defaults). HONEST SCOPE: densities are MOON-hardcoded (constants.py RHO_DEEP/RHO_SPOIL); body-aware densities are deferred (task #62), so V=D is for LUNAR planning only. [SHIPPED: gis/qwc2/js/mission/planTools.js + planTools.test.js; densities moon-hardcoded, body-aware is task #62] | D | D | D | NA |
| GW-01 | P1 | 1 m LROC NAC regional imagery: a site NAC drape (0.5-2 m) loads as a Trek-WMTS/COG layer in the local polar-stereographic frame, registered to the LOLA DEM within tolerance; whole-moon basemap stays WAC/Kaguya. (extends GW-05; task #47) | N | N | N | NA |
| WS-01 | P1 | A permanent top World-State header strip on every IDE screen shows mission, twin-sync, terrain-version, changed-since-last-mission, confidence, learning-dataset-size, pending-validation, sourced from /twin/version + DT-05 observed-fraction + mission-time; the 2 new counters bind real endpoints or are marked unavailable (no fabricated count); a test asserts the fields render with real values. (extends DT-05) | N | N | N | NA |
| WS-02 | P1 | A Digital-Twin Health panel shows per-model agreement/trust (localization/map-agreement/terrain/wheel/battery/excavation) computed from real predicted-vs-observed telemetry (TM-04) or explicitly flagged not-yet-surfaced; a test asserts each shown percent traces to a real computation (no fabricated value). (wires TM-04) | N | N | N | NA |
| WS-04 | P1 | A Learning-lifecycle panel lists collect-experience -> generate-dataset -> retrain(traversability/slip/energy) -> evaluate-policy from REAL roversim RL-stack artifacts (no synthetic), and blocks any policy from an operational label until it carries the RL-01 card; a test asserts real lineage renders and an RL-01-incomplete policy is blocked. (UI complement to CR-01 + RL-01) | N | N | N | NA |
| WS-05 | P1 | A future-comparison tradeoff card renders >=2 candidate futures side-by-side (kWh/hours/slip/recharge/completion) bound to real lode.resync.forward_compare / FL-06 output; a test asserts the card binds real forward-compare rows (no synthetic). (surfaces FL-06) | N | N | N | NA |
| TP-01 | P1 | A Mission Tasks palette leads with work-packages (LandingPad/HaulRoad/Berm/...) grouped into 6 functional groups (Earthworks/Transportation/Construction/Survey/Science/Fleet), two-level (Primitive Ops + Templates); selecting a work-package emits a typed order that decomposes to mass-balanced cut/fill via SD-01; Playwright asserts the 6 groups render and a template selection produces a decomposed order. (extends SD-01/CP-04; task #64) | N | N | N | NA |
| TP-02 | P1 | An intelligent template wizard: selecting e.g. Landing Pad collects (Length/Width/Bearing/Finish/Flatness/Elevation/Material/Priority), computes cut+fill polygons+equipment+routes+time+energy+risk, and expands to the Survey->Excavate->Move->Grade->Compact->Inspect->Approve work order; a test asserts a parameterized template yields the expected mass-balanced volumes + ordered steps. (distinct from FS-30 plan wizard) | N | N | N | NA |
| GW-09 | P2 | A dual-mode planning graticule (selenographic lon/lat densely sampled + reprojected so it curves in polar-stereographic, plus a straight metric km-grid with labels) overlays the lunar map given an injected reproject(lon,lat)->[x,y]; a node test asserts gridline + label generation. (extends GW-05) [REQ:GW-09] `stewie/server/test_gw09_graticule.py` (the python static gate req_trace counts) + `gis/qwc2/js/mission/graticule.test.js` (the node runtime proof, 8/8): graticule.js generates BOTH modes — selenographic meridians/parallels sampled densely then reprojected through an injected reproject(lon,lat)→[x,y] (so they curve in polar-stereographic) with degree labels, and a straight metric km-grid with km labels — and guards an off-map reproject (NaN/undefined/throw via `_safe`, dropping the point + any <2-point line) so no malformed polyline reaches the pole; Graticule.jsx overlays it as an OL vector layer injecting the real proj4 IAU_2015:30100→30135 (NOT OL's Earth-datum graticule). I=D (graticule.js meridians/parallels/kmGrid + _safe/_range guards, pure + node-testable), X=D (Graticule.jsx cycles off→selenographic→metric via G.selenographic/G.kmGrid + map.addLayer; the node test runs in the CI JS tier, the static gate in the python suite), V=D (the node test proves gridline + label generation + the reproject guards at runtime; the python static gate asserts both generators + the labels + the guard + the plugin's lunar-reproject overlay, so req_trace's python scan counts it). [SHIPPED: gis/qwc2/js/mission/graticule.js + graticule.test.js] | D | D | D | NA |
| LY-04 | P1 | The missing layer kinds (ice-probability, localization-confidence, sensor-coverage, digital-twin-difference) each register in the LY-01 catalog with a REAL backend producer + provenance/eligibility, or are explicitly marked unavailable (no fabricated drape); a test asserts each is typed+registered or explicitly absent. (extends LY-01) [REQ:LY-04] `stewie/server/test_ly04_missing_layer_kinds.py` binds each of the four PRD-named kinds to an honest status via `gis_layers.missing_layer_kinds()` and proves it: digital-twin-difference is AVAILABLE with a REAL producer (the LY-07 `changed_terrain` signed as-built-minus-base drape, allow-listed in `_GLOBE_KINDS` with a legend and bound to the catalog rows map.changed_terrain + evidence.before_after_dem), verified by rendering a NON-transparent worked change on the REAL Haworth DEM after a conserved cut+fill (no synthetic terrain); the other three are EXPLICITLY UNAVAILABLE with no fabricated drape — ice-probability (terrain.thermal catalog-only, no Diviner/thermal ice raster; terrain.psr is the real cold-trap proxy), localization-confidence (robot.covariance/map.uncertainty are belief/live SLAM/EKF channels, no per-cell producer over the fixed prior DEM), sensor-coverage (robot.sensor_frustums is a live/sim channel; the LOS/viewshed producer is tracked as the still-unbuilt LY-06). I=D (the registry + the four honest statuses live in `stewie/server/gis_layers.py::missing_layer_kinds`, provenance/eligibility read from the committed catalog, never fabricated), X=D (surfaced additively on `/world/layer-catalog` as `missing_layer_kinds` so the status is discoverable at the LY-01 catalog surface; the LY-01 count/sync is untouched), V=D (the [REQ:LY-04] test asserts exactly the four kinds, each producer-real-rendering-on-the-real-DEM or explicitly-unavailable-no-drape, and that unavailable kinds are neither allow-listed nor claim a producer). | D | D | D | NA |
| SD-03 | P2 | Resource-exploration cross-section: a drawn transect returns an elevation profile with PSR/ice-stability + regolith/bearing draped along it, each value traced to its source layer + uncertainty. (extends SD-01; task #45) | N | N | N | NA |
| TP-03 | P2 | A context-aware palette: available tools filter to the SELECTED vehicle's capabilities from the real VehicleModel registry AND to a selected existing feature's type/state (a berm -> Inspect/Extend/Repair/Remove/Compact/Survey); a test asserts an Excavator exposes Dig/Dump/Compact/Grade (a Survey rover does not) and selecting a berm yields its action set. (extends VT-01) | N | N | N | NA |
| TP-04 | P2 | An object palette writes an expanded place-object vocabulary (Beacon/Instrument/Sample/Antenna/Habitat/PowerStation/Relay/Charger/StorageBin/ProcessingPlant/NavMarker) as durable first-class world objects in the versioned twin (survive reload, carry provenance); a test asserts a placed object round-trips through the twin store. | N | N | N | NA |

### 7.E Deck-forward architecture — walkthrough gaps (2026-07-07)

The 14-layer architecture walkthrough deck (`design/STEWIE_deck_vs_current_diff_2026-07-07.md`) runs ahead of the matrix on its forward layers. A two-agent deck-vs-PRD audit (L1-L7 + L8-L14/App, each row grounded at a PRD line) found the **deterministic core fully captured** (data sources, world-state, sim/twin, export; PostGIS = PG-01, rock detection = ML-03, analytical terramechanics = TM-01/02) and **15 genuine gaps** — folded here as tracked rows so the /program board reflects the whole deck. All `N | N | N | NA` (ADDITIVE — no glyph churn to delivered work). Groups: extended storage, standards serving, the deck's bounded learned-ML insertion points (sequenced AFTER the deterministic core per "ML proposes · physics constrains · STEWIE reconciles · ROS executes"), ingestion, thin mission primitives, and onboard fusion.

**Loop pick order — DERIVED ([REQ:PO-19]):** `python scripts/atg.py`. (The learned-ML + onboard rows still trail the deterministic core, but that ordering is now a consequence of the declared prerequisites rather than a transcription that drifts.)

| ID | P | Requirement and acceptance | I | X | V | Q |
|---|---|---|---|---|---|---|
| SC-01 | P2 | **STAC catalog/index.** A STAC catalog indexes terrain/DEM/imagery/layer assets (items+collections with provenance/CRS/freshness) backing the LY-01 registry; a bbox/time/collection query returns matching asset records. (extends BA-11/PG-01) | N | N | N | NA |
| SC-02 | P2 | **STAC object store.** Ingested rasters/vectors persist to a STAC-indexed object store (COG/asset blobs + catalog) so a layer resolves by STAC item id; a durable projection, never authority. (extends SC-01/PG-01) | N | N | N | NA |
| TS-01 | P2 | **TimescaleDB telemetry.** A TimescaleDB (or equivalent hypertable) durably persists mission/rover telemetry time-series with a retention policy; a time-window query returns telemetry within a latency budget; a projection, never authority. (extends PG-01) | N | N | N | NA |
| PD-01 | P2 | **PDAL point-cloud ingestion.** PDAL loads external point-cloud products (LAS/LAZ/PLY) into the source-neutral cloud contract with CRS/frame validation, feeding the PM-13/PM-14 DepthObservation path; a real point-cloud fixture round-trips bounds/CRS. (extends PM-14) | N | N | N | NA |
| TT-01 | P2 | **TiTiler raster serving (alternative, not adopted).** A TiTiler dynamic-COG tiler would serve site DEM/imagery raster tiles on demand as an ALTERNATIVE to the DEPLOYED QGIS-Server WMS (QG-02, deploy/compose.yml:251); a tile request returns a correct lunar-frame tile within tolerance; adopt-vs-keep-WMS is a recorded decision (WMS is the incumbent). (extends GW-05/FR-12) | N | N | N | NA |
| OF-01 | P2 | **OGC API Features serving.** An OGC API Features endpoint serves mission vector layers (keep-outs/routes/zones/targets) as standards Features in the lunar CRS, augmenting the custom `/world` routes; a client fetches features by bbox/collection. (extends BA-11/GI-03) | N | N | N | NA |
| HF-01 | P1 | **Bayesian hazard fusion.** Hazard/costmap fusion combines rock/slope/sinkage/slip/illumination/change cues via a probabilistic (Bayesian) posterior hazard map with calibrated per-cell uncertainty, not only a rule/physics-weighted sum; the posterior + uncertainty drive the costmap. (extends AS-11) | N | N | N | NA |
| ML-10 | P1 | **Crater / negative-obstacle detector.** An image/depth model produces crater/pit location, size, and confidence for navigation, acceptance-gated against held-out truth; complements the geometric `negative_obstacle_mask` + PM-14 pit recognition. (extends ML-03/PM-14) | N | N | N | NA |
| PX-07 | P2 | **Learned terramechanics surrogate.** A learned surrogate implements the PhysicsBackend protocol (sinkage/slip/traversability/energy faster than the analytical Bekker solve), calibrated to the analytical/Chrono oracle with bounded uncertainty + a deterministic analytical fallback; advisory/planning-only until validated (RL-01/TM-01), never sole release authority. (extends PX-04/TM-01) | N | N | N | NA |
| PX-08 | P0 | **Low-gravity Bekker-modulus provenance is single-sourced and never double-corrected.** The shipped Bekker set (`stewie.specs.constants`: k_phi=820000 Pa/m^n, k_c=1400, n=1.0, c=170 Pa) IS the NASA LTV terramechanics white paper's LUNAR reference (NTRS 20220010732) -- it already encodes the 1/6-g condition. Therefore the lunar parameter set is `TerramechanicsParams.from_constants()`, **unreduced**, and applying the Lyasko-2010 1g->1/6g correction on top double-counts gravity. Acceptance: one stated provenance (constants.py + the stewie_forge/terramechanics.py header + body_params.py PHYS-01 must agree, not contradict), NO constructor that hands back a pre-reduced 'lunar' set, and NO production caller of `lyasko_reduce` (the transform stays available, correctly, for an EARTH-fit modulus set). [REQ:PX-08] `stewie/physics/test_lowg_modulus_provenance.py` asserts all four on the real shipped constants: (a) they equal the sourced NTRS lunar set; (b) the drive path's default params are that set and are NOT pre-reduced (lyasko_reduce still strictly lowers k_phi/cohesion from them, so they cannot already be reduced); (c) `TerramechanicsParams.lunar()` does not exist; (d) an AST guard finds zero production callers of `lyasko_reduce` across stewie/, packages/stewie-forge/, lode/, dart/, leap/. NON-VACUOUS: the guard is verified to FAIL on a re-introduced production reduce, and the suite was RED on the footgun before it was removed. WHY P0: the deleted `.lunar()` returned k_phi 614624 (-25%) / cohesion 127.4 (-25%) while its docstring said "Use for lunar runs" -- it UNDERSTATES the frictional modulus and so OVERSTATES sinkage, slip and entrapment on EVERY absolute terramechanics number the product reports. It had zero production callers, so the shipped figures were always correct and NONE are renumbered by this fix; what changed is that the correct behaviour is now stated and enforced rather than accidental. I=D (footgun removed; provenance corrected in constants.py, the forge header, rover.py's docstring; the three tests that exercised the transform through `.lunar()` now call `lyasko_reduce()` directly and no longer misname its input "earth"), X=D (the drive path -- rover/drive/slip -> from_constants() -- and body_params.py PHYS-01 now agree with the documented source), V=D (the four [REQ:PX-08] gates above, non-vacuity demonstrated). (resolves DEFERRED_FIXES FIX-6; settles the constants.py-vs-body_params.py contradiction; extends PX-04/PX-06) | D | D | D | NA |
| PX-09 | P0 | **The live dig is bounded by the drum's own sourced limits (anti-bridging bite + drum hold).** `Viz2Runtime._apply_dig` cut a fixed `dig_depth_m` box and booked whatever mass came out, consulting neither [BDSCALE] limit, which broke the excavation model twice over. (1) **Energy and terrain described different bites:** `_dig_fee` already clamped its characteristic depth to `max_cut_per_pass_m(drum)` (= MAX_CUT_DEPTH_FRAC 0.50 x the drum's scoop-opening height, the anti-bridging rule), but the CUT ignored that cap -- so an over-deep pass removed more mass than a legal one while being billed at the shallower legal depth, and the McKyes/Reece FEE rises with depth^2, so dig energy was UNDERSTATED exactly when the cut was worst. (2) **The drum had no capacity:** `DRUM_CAPACITY_KG` (small 3.80 / medium 7.30 / large 24.98 kg) was never consulted, so the drum filled without bound -- measured on the real Haworth SfS bundle, 20 passes with the small drum booked **73.2 kg into a 3.80 kg drum (19x over) on a 30 kg-class vehicle**, and there was no fill->stop->haul quantum at all. Now that the Drive 3D console is publicly reachable (RT-06), an anonymous operator could do exactly that. Acceptance: the cut depth per pass is capped at the anti-bridging limit so the terrain and the FEE describe the SAME bite; the drum ledger never exceeds its sourced hold (a bite that would overfill is trimmed to the room left, using the height identity dz*rho*area as an UPPER bound on what moves -- `cut_to_inventory` also clamps at the firm DEM datum -- so trimming can only under-fill, never overfill); a FULL drum REFUSES the bite (no mass leaves the grid) -- that refusal IS the haul quantum; mass stays conserved across a capped cut; and the drum's fill/capacity/full state + the bite cap are published as telemetry so the operator can see WHY digging stopped. [REQ:PX-09] `stewie/runtime/test_viz2_dig_limits.py` asserts all of it against the REAL Haworth SfS 1 m bundle through the real `Viz2Runtime` (constructed, actor loop not started -- no synthetic terrain, no faked timing): an over-deep request (50 mm on the small drum, ~4x its 13.2 mm limit) cuts no deeper than the cap; the mean bite the terrain gives up is <= the depth the FEE bills; 60 passes never exceed the 3.80 kg hold and a further pass then moves NOTHING; grid mass lost == drum mass gained across a capacity-limited cut; and every drum's hold is <= the 30 kg/cycle RDS envelope, so that envelope now holds BY CONSTRUCTION instead of being flagged after the fact. Real TDD: the suite was RED on all of these before the fix. NOTE the default drum is `large` (23.9 mm cap) and the shipped `dig_depth_m` is 20 mm, so the DEFAULT bite was already legal -- the bite cap binds for the small/medium drums and any deeper `dig_depth_m`, while the capacity cap binds for every drum. (The WorkSite `flatten` PRIMITIVE stays unbounded on purpose: a planner cut order is not a single drum pass.) I=D (both caps enforced in `_apply_dig` + drum telemetry), X=D (bounds the live dig on the deployed public Drive 3D console; the FEE's own clamp is now a consequence of a legal cut rather than a mask over an illegal one), V=D (the five [REQ:PX-09] gates above on real terrain). (sourced: Schuler et al., "ISRU Pilot Excavator: Bucket Drum Scaling" [BDSCALE]; bounds RT-06; extends PX-04) | D | D | D | NA |
| PX-10 | P1 | **The front-arm DOF is physical: the arm posture gates the cut.** The arm was a COSMETIC render offset -- `viz2_root.gd` kept `_arm_front_offset` only to pose the render joint, never told the runtime about it, and `_apply_dig` cut its full `dig_depth_m` regardless. So a rover driving with its drum RAISED FOR TRANSPORT still carved a trench under itself, and the operator's arm control was a lie: it moved the picture and nothing else. On a HITL command surface -- now publicly reachable (RT-06) -- the pictured vehicle state and the world state must not disagree. Acceptance: the arm is AUTHORITATIVE state that the physics reads (the rig forwards the pose over the existing runtime command channel as `{cmd:"arm"}`); the cut is gated by a dig ENGAGEMENT computed from the posture the rig itself poses (effective pitch = `ARM_DIG_DOWN_RAD` + the operator's offset) -- 1.0 at the dig posture, 0.0 at/above stowed; drum up for transport removes NO mass; between the postures the arm MODULATES the bite (still bounded by the PX-09 caps); offset 0 IS the dig posture, so the DEFAULT dig is unchanged and arming the gate cannot silently break every existing dig; arm commands off the public console are bounds-checked at ingest like the twist (M-04) -- non-finite is REFUSED outright, wild angles clamp to the rig's travel, so a poisoned angle can never license a deeper cut; and the arm pose + engagement are published as telemetry so the operator can see WHY a dig carved nothing. [REQ:PX-10] `stewie/runtime/test_viz2_arm_gate.py` asserts all of it on the REAL Haworth SfS 1 m bundle through the real `Viz2Runtime`: engagement is 1.0 at the dig posture / 0.0 stowed / 0.0 on NaN; a transport-posture dig moves ZERO mass and books ZERO regolith; the default dig still cuts; a half-raised arm takes a strictly shallower bite than a fully-lowered one (measured: 15.83 kg down / 9.68 kg half / 0.00 kg up); and a 1e9-rad command clamps instead of digging deeper. It ALSO pins the seam that made this bug possible: a DRIFT GUARD asserts `viz2_root.gd`'s ARM_DIG_DOWN/ARM_OFFSET_MIN/MAX still mirror `stewie/specs/arm_state.py` (the ONE authority the gate reads), and a WIRING guard asserts the rig actually calls `send_arm` and the client still speaks `{cmd:"arm"}` -- because a gate the rig never feeds is worthless, which is exactly what the old code was. Both non-vacuous (verified to FAIL on a drifted constant). Real TDD; Godot `--check-only` parse gate clean on both scripts. HONEST MODELLING NOTE: the physically right engagement is geometric (drum-edge penetration = pivot height - L*sin(pitch) - drum radius), but there is NO sourced arm-pivot height above ground (assuming the pivot sits at the wheel axle puts the large drum's edge BELOW the wheels at stow, which is plainly wrong), so inventing that geometry would fabricate every absolute number falling out of it. The engagement is instead a LINEAR ramp between the two postures the sim itself defines; the ramp SHAPE is [ASSUMPTION], while the GATE it implements (arm up => no cut) is exact and IS the requirement. A geometric model is deferred to a sourced pivot height. I=D (authoritative arm state + ingest + gate + telemetry in the runtime; `send_arm` in the drive client; the rig forwards the pose), X=D (bounds the live dig on the deployed public Drive 3D console; the render rig and the physics authority now agree on where the arm is), V=D (the [REQ:PX-10] gates above on real terrain + the drift/wiring guards). (bounds RT-06; composes with PX-09; extends the arm_state T2.1/T2.2 model) | D | D | D | NA |
| PX-11 | P1 | **The excavation footprint is a PHYSICAL dimension (the drum), so a render setting cannot resize the vehicle.** The viz2 setup page offers a "cell size" choice (5 cm / 2 cm) that every operator reads as a RENDER-RESOLUTION knob. It was not one: `_apply_dig` built its box from `dig_half_cells = 6` -- a CELL COUNT -- so a finer grid silently shrank the excavator. Measured on the REAL Haworth SfS bundle: the same 13x13 box is **0.650 m at 5 cm but 0.260 m at 2 cm**, so ONE dig command on IDENTICAL terrain moved **15.832 kg vs 2.231 kg -- 7.1x more mass from a display setting alone** -- and NEITHER box was the drum's real width (0.3526 m; 1.84x too wide / 0.74x too narrow), while `_dig_fee` billed the pass with `width_m = the REAL drum width`. Cut geometry and energy model described different machines: the PX-09 class of self-contradiction, one level further out. Acceptance: the dig box side is DERIVED from `DRUM_DIMENSIONS_M[drum]["width"]` (closest odd cell count), so the footprint matches the sourced drum within one cell at every offered resolution, the toggle no longer changes the machine, one dig moves comparable mass at either resolution (grid quantisation only), and the tool that CUTS is the tool the FEE BILLS. Now: 0.350 m at 5 cm / 0.340 m at 2 cm (0.99x / 0.96x of the drum), mass ratio across the toggle **1.05x, was 7.10x**. [REQ:PX-11] `stewie/runtime/test_viz2_dig_footprint.py` asserts all four on the real bundle. Real TDD (RED on all four first). **It also exposed a flaw in PX-09's own implementation, now fixed:** the anti-bridging cap anchored the cut level to the box MINIMUM (`target = box_min - d_eff`), but `flatten` cuts every masked cell down to that level -- so a cell standing PROUD of the box gave up `(its relief) + d_eff`, exceeding the cap on any uneven ground. The PX-09 test only passed because the oversized cell-count box cut enough mass that the drum-capacity trim raised the target and accidentally masked it; shrinking the footprint to the real drum removed the mask. ONE `{dig}` is ONE PASS and the rule bounds the DEEPEST bite entering the scoop, so the level now anchors to the box MAXIMUM: the deepest per-cell bite is exactly `d_eff`, the cap binds per-cell (verified: a 50 mm request on the small drum now cuts exactly 13.2 mm, its limit), and a pass SHAVES the high spots while repeat passes progressively deepen the rut -- what a bucket drum at a fixed cutting level physically does. A green suite was necessary, not sufficient. I=D (footprint derived from the drum; per-cell cap corrected), X=D (the live public Drive 3D console's "cell size" control is now a pure display choice), V=D (the [REQ:PX-11] gates + the corrected [REQ:PX-09] gates, all on real terrain). (fixes a flaw in PX-09; composes with PX-10; sourced [BDSCALE]) | D | D | D | NA |
| CG-01 | P2 | **Learned cost-fusion.** A learned model produces costmap term-weights from recorded operator/demonstrated paths; every learned weight is attributable to its training source and the fused cost decomposes into inspectable terms; the deterministic rule-weighted costmap remains the fallback. (extends SN-05/AS-11) | N | N | N | NA |
| SV-01 | P1 | **Survey mission order.** A first-class typed survey order in the goal grammar (footprint + coverage/re-observation intent) compiles to a coverage route (NV-02) + Observe/MEERKAT actions (AM-01); acceptance authors a survey order and drives it end-to-end. (extends CP-04/NV-02) | N | N | N | NA |
| RO-01 | P2 | **Reorient primitive.** A reorient primitive (commanded in-place heading/posture change as an authorable order) lowers to a bounded twist (NV-04) under stability guards (VT-06); acceptance emits a reorient with no net translation and a validated stability margin. (extends CP-04) | N | N | N | NA |
| QB-01 | P2 | **Quantized shared backbone.** A single quantized shared vision backbone feeds the onboard perception heads (rock/depth/hazard/VO) within the ML-09 edge envelope; acceptance measures shared-backbone RAM/latency on the named compute profile and asserts head outputs match the separate-model baseline. (governed by FS-12/RL-01) | N | N | N | NA |
| CR-01 | P2 | **Scheduled retraining trigger.** A scheduled trigger turns EG-08/MP-11 divergence + execution-feedback logs into a curated (no-synthetic) training set, retrains the fine-tuned nodes, and gates every candidate through RL-01 + FS-12 before redeploy; a divergence threshold enqueues a retrain job blocked until the gates pass. (extends EG-08/MP-11) | N | N | N | NA |
| TF-01 | P2 | **Onboard TSDF fusion.** An nvblox-style TSDF accumulates the selected depth source (PM-13) into a 3D SDF within the ML-09 Jetson envelope, reconciled against the conserved 2.5D elevation map (PM-08/AS-10); truth-denied; the live device is the gated leg (RS-06). | N | N | N | NA |
| GW-11 | P1 | **3D terrain view in the /ide, synced to the GIS map.** A Three.js 3D terrain panel in the QWC2 IDE renders the selected site's REAL DEM window (via `/dem/heightfield`) in the site-local frame with a vertical-exaggeration control (1-5x, labeled display choice); it reuses/ports the cockpit `three3d.js` capabilities (orbit/fly, sun shadows, layer-texture drape) rather than a new renderer. Click-3D returns the exact map coordinate (IAU_2015:30135 + selenographic, via the shared `/dem/site_lonlat` transforms) into the IDE coordinate display; a mission feature authored on the 2D map (waypoint/keep-out/work-zone) appears in the 3D view from the same backend state within one refresh, and a 3D waypoint pick lands in the same order-frame serializer the 2D path uses. [REQ:GW-11] `stewie/server/test_gw11_terrain3d_ide.py` + `gis/qwc2/js/mission/gw11_terrain3d_acceptance.test.js` assert the acceptance on the REAL Haworth 5 m DEM (no synthetic terrain): the backend test proves the three /dem contracts the panel composes — (a) /dem/heightfield_full[/meta] serves the real site-local DEM window (native 5 m, whole-tile order frame, >1 km relief, binary == n·n float32), (c/e) /dem/site_lonlat maps an order-frame point to a real south-polar selenographic lat/lon (agreeing with dem_origin_to_latlon; 422 out-of-window) — the shared transform the hover readout AND the 3D pick both call, and (d) /dem/site_meta.bounds_m is the IAU_2015:30135 anchor {x0=-52900 min X, y1=105400 max Y} the 2D-authored features convert THROUGH (order_x=X−x0; order_y=y1−Y, the pixel-vs-node one-cell offset asserted); the node test binds the three pure serializers — formatHover (order metres E/N + selenographic lon/lat, dash never fabricated), featuresToSpecs (30135 keep-out/marker → exact order-frame ring/point), and the WS features+plot channels (the authored set HELD + notified within one refresh; a 3D pick reaches the SAME WS.emitPlot channel the 2D singleclick fills). I=D (viz3d.js → window.STEWIE_VIZ embedded via gis/qwc2/js/plugins/MissionTerrain3D.jsx: full-res /dem/heightfield_full relief in the site-local frame, 1-5x vex, ported three3d.js caps — orbit/fly, sun az/el shadows, draped layer stack; hover/plot raycast the mesh for UV-derived order-local e_m/n_m — flat/globe-invariant — + selenographic lon/lat; 2D features drape as red keep-out rings + markers via renderMissionFeatures), X=D (the panel mounts in the QWC2 IDE, syncs to the GW-02 workspace site, wires WS.onFeatures for 2D→3D + VIZ.onPlot→WS.emitPlot for 3D→plan; both [REQ:GW-11] tests run in CI — the python suite + the JS tier), V=D (the deployed /ide is Playwright-verified on real GPU: hover reports `E 5579.2 m N 5579.2 m` / elev / `lat -86.35829° lon -25.36303°` — frontend/_ide_hover_e2e.mjs; 24 keep-outs authored on the 2D map render as 24 rings in the 3D view within one refresh — frontend/_ide_features_e2e.mjs). (extends QW-01/GW-05/GW-02; reuses stewie/server/web/assets/three3d.js + routers/dem.py heightfield) | D | D | D | NA |
| GW-12 | P0 | **Planet-fixed-authoritative + local-render-origin coordinate contract (single-precision safe).** One documented, tested contract: authoritative coordinates exist ONLY in the lunar CRSs (IAU_2015:30135/30100, or the ad-hoc tile's local CRS) on the server; every render surface (cockpit 3D, /ide 3D, Cesium globe, Godot) declares a local origin near the work area and renders float32-relative to it; every pick/edit/measure converts back through `site_dem.py` transforms (latlon_to_dem_origin/dem_origin_to_latlon) before persisting, never storing a renderer-local frame. Acceptance: an executable [REQ:GW-12] test asserts (a) coordinate round-trip error under 1 cm at the 30135 theme-extent corners, at each imported site anchor, and at an ad-hoc tile center; (b) the largest coordinate magnitude handed to a float32 render path stays under a stated bound (documented ulp budget); (c) a guard that no gis/qwc2/js or web/assets module persists coordinates without the shared serializer. [REQ:GW-12] `stewie/server/test_gw12_coordinate_contract.py` verifies all three on REAL committed DEM metadata: (a) the render->pick->persist round-trip is 0.0 mm (< 1 cm) at the theme-extent corners, tile center, and site anchor of every curated tile (haworth/shackleton_rim/nobile_rim/de_gerlache_kocher) AND at an ad-hoc local-AEQD tile center; (b) the worst render-origin-relative extent (~15.1 km incl. relief) has a float32 ULP of 0.98 mm < 1 cm, under the documented 2^17 m (131 km) 1cm-safe bound with an 8.69x margin; (c) the grep/AST guard finds 0 renderer-local persists (the one render-origin producer viz3d/frame.js never persists; the shared orderFrameEntry/markerBody serializers consume map-CRS coords; the 3D-pick bridges reproject lon/lat->map CRS). I=D (site_dem transforms + the shared serializer are the sole persistence path, live on /api/plan + the cockpit/ide picks), X=D (the contract governs the delivered cockpit 3D, /ide frame.js, and the plan-tool serializers), V=D (the [REQ:GW-12] test is non-vacuous: the guard fires on a raw render-origin persist and clears once a metresToLonLat/reproject bridge is inserted). (formalizes site_dem.py:211-266 + planTools.js:99-115; extends GW-02/GW-05) | D | D | D | NA |
| PX-12 | P1 | **The dump is METERED by the drum's own scoops — a berm is built by passes, not by one instantaneous mound.** Digging became genuinely physical (PX-09 capacity + the ≤50%-scoop anti-bridging bite, PX-10 the arm gates the cut, PX-11 the footprint IS the drum). Dumping stayed crude: `_apply_dump` called `ws.dump(mask)` with **no kg**, discharging the **ENTIRE drum ledger in a single frame** — up to 24.98 kg teleporting onto a 0.35 m box in one tick, after which the sandpile relax spread the resulting mound. A bucket drum cannot do that; it EMPTIES the way it FILLS, through its scoops, a bite at a time. Acceptance: one dump command is ONE PASS OF SCOOPS, bounded by `drum-width footprint (PX-11) × the ≤50% anti-bridging bite (PX-09, [BDSCALE]) × in-situ density`; emptying a full drum takes a COUNTABLE number of passes and drains monotonically; the metered dump still conserves mass exactly; an empty drum dumps nothing; and a metered dump still BUILDS (the surface under the drum rises). Measured (5 cm cell): quantum **small 0.39 / medium 1.40 / large 3.81 kg** → **9.8 / 5.2 / 6.6 passes to empty**. Passes-to-empty is deliberately NOT monotonic in drum size — the footprint quantises to whole cells (PX-11), so the small drum's box is proportionally smaller relative to its hold; the quantum FALLS OUT of the sourced geometry and is not tuned. [REQ:PX-12] `stewie/runtime/test_viz2_dump_limits.py` asserts all six on the REAL Haworth SfS bundle, including a SYMMETRY gate that pins the model to the dig: a discharge pass carries what a dig pass carries across all three drums, so if the sourced bite or the drum footprint ever changes, the dump MUST move with it and cannot silently drift apart. Real TDD (RED first on the three metering assertions). HONEST: the symmetry itself (a discharge pass carries what a dig pass carries, because it is the same scoops turning the other way) is the **[ASSUMPTION]**; the GEOMETRY it rests on is sourced. IMPLEMENTATION NOTE: the conserved authority ALREADY accepted a `kg` (`WorkSite.dump(mask, kg=…)`) — the runtime simply never passed one, so this is a bounding of an existing conserved primitive, not a new physics model. What it buys is the thing that was missing for training: a dump is a FINITE, REPEATABLE quantum you can plan, cost, and learn from. I=D (quantum derived in the runtime from drum geometry; the dump meters against it), X=D (bounds the live public Drive 3D console — a berm now takes real passes), V=D (the six [REQ:PX-12] gates on real terrain). (needs PX-11; mirror of PX-09; prerequisite for PX-13) | D | D | D | NA |
| PX-13 | P1 | **Both counter-rotating drums dig — the chassis reaction nets ~0 (KSC-TOPS-7), and the back drum stops being a dead DOF.** The sim modelled HALF THE MACHINE. `_drum_rc()` resolved only the FRONT drum, so `_apply_dig` cut at one footprint and billed the chassis the FULL draft `F = tau/r` — the CORRECT answer for a single-drum dig, and the WRONG VEHICLE. The real RASSOR/IPEx digs with BOTH bucket drums counter-rotating; per **KSC-TOPS-7** their horizontal reactions are equal and OPPOSITE, so the pair nets ~0. In ⅙ g that cancellation is not an optimisation — it is the only way a 30 kg-class rover can react the digging force at all, because it lacks the weight to shove back. So the sim was pushing the rover backward with a force the real machine is **built to cancel**, and that false force flowed into traction, slip and energy on EVERY dig tick — which is why this is a **prerequisite for the training rows** (PX-14, TR-02), not a polish item: a policy trained against it learns to compensate for a reaction that does not exist. **The back drum was a DEAD DOF** — `_arm_back_offset_rad` was set, ingested from the operator, and published as telemetry, and never reached physics (PX-10's cosmetic-arm bug, on the other arm). Each arm now gates ITS OWN drum, so the cancellation happens for the RIGHT REASON (two drums actually cutting) and stowing one restores the full draft rather than zeroing a term. **HONEST CORRECTION, screened before TDD:** the first framing claimed the live dig "does not use `net_dig_reaction_n`". FALSE — it did, correctly, with `drums=("front",)`. The defect was never a miscalled model; it was that the sim only ever modelled one drum. Acceptance, with the strongest gate the physics allows: on REAL uneven terrain the pair does NOT cancel to exactly zero — the two drums bite different material, and pretending otherwise would be the lie — so the residual has an EXACT prediction, **reaction(both) == |draft(front only) − draft(back only)|**, which is far harder to fake than `< epsilon`. Measured on the real Haworth SfS bundle: **front only 1.970 N · back only 1.691 N · both 0.279 N == |1.970 − 1.691|** → an **86% cancellation**, and the mass ADDS (0.387 + 0.335 = 0.722 kg — two drums, two footprints). Also: two separate contact patches straddling the pose (±0.40 m along the heading); each drum still bounded by PX-09 (anti-bridging + the now-SHARED hold), PX-10 (its own arm gate) and PX-11 (the drum-width footprint) — one implementation driven twice, so the drums cannot drift apart. **[BDSCALE] capacity is PER DRUM** ("Avg total regolith collected per drum", Schuler 2022 Table 3), so the VEHICLE hold is 2×. What follows is NOT what it first appears: mass-in doubles AND the tank doubles, so **passes-to-full is UNCHANGED**; what doubles is **THROUGHPUT** (every haul carries twice the regolith), which is the actual reason the machine has two drums. **A SOURCED TENSION, SURFACED NOT CLAMPED:** at two drums the hold is small 7.60 / medium 14.60 / **large 49.96 kg** — and PX-09 asserted every hold sits inside the 30 kg/cycle RDS envelope "by construction", which is now **FALSE for the large drum**. It is also a category error that predates this row: `ipex_specs` says plainly "IPEx uses the small..medium range; **the large drum is the RASSOR 2.0 drum**", so the IPEx RDS envelope never applied to it. The test ASSERTS the breach rather than hiding it — a spec mismatch you can see is worth more than one you cannot. [REQ:PX-13] `stewie/runtime/test_viz2_dig_reaction.py` (6 gates). NON-VACUOUS: neutering the counter-rotation (both drums same sign) turns it RED (verified). **Three sibling gates were UPDATED, not loosened** — PX-10's transport test raised only the FRONT arm and expected a zero cut (right for a one-drum sim, wrong for the vehicle: a hauling rover stows BOTH), its poison-resistance test now bounds BOTH commandable arms, and PX-09's cap is now the VEHICLE hold; a new corollary gate asserts that stowing ONE arm still digs with the other, so nobody can 're-fix' the transport test by re-collapsing the two drums into one. I=D, X=D (bounds the live public Drive 3D console), V=D. (needs PX-12; unblocks PX-14 → TR-02; the model already existed in `arm_state.net_dig_reaction_n` and the dig path only ever passed it one drum) | D | D | D | NA |
| PX-14 | P1 | **AutoDig-equivalent CLOSED-LOOP trench control — the bite is regulated from the drum-current observable, and the runtime finally EXPOSES that observable.** NASA's **AutoDig** (the semi-autonomous excavation routine for RASSOR → IPEx) maintains ground contact and **regulates digging depth from the drum motors' current draw** — from how much soil the drums are actually ingesting. STEWIE had the SENSOR and not the LOOP, and the runtime did not even expose the reading: `rassor_mass_model` grounds the Free-spinning Drum Current (mass ∝ steady current; `FDC_LINEAR_R2 = (0.989, 0.985)`, sourced from **NTRS 20210022781**) and can both synthesise the current from conserved drum mass and infer mass back with the published band — but the live dig was a COMMANDED VERB (`cmd == "dig"` → `_apply_dig()`, a fixed depth): no controller, no feedback, no termination, and no `drum_current_a` in telemetry. Another sourced model with nothing wired to it. **HONEST SCOPE, measured not assumed:** the conserved sim's density field is **UNIFORM** (std 0.000 kg/m³ over the fine window), so there is NO real gradient to regulate a *"denser → shallower bite"* against — inventing one would be fabricated data, so this row does **NOT** claim density adaptation. It claims the loop the physics genuinely supports: the bite is regulated to a **target per-pass ingestion**, read through the drum-current OBSERVABLE (with the FDC noise band) rather than the conserved truth a real rover cannot see; the trench **terminates on drum-full via the sourced `should_offload` trigger**, on the UPPER confidence bound, so it never overfills even when the sensor is noisy; and the regulated bite stays inside every sourced bound (**PX-09** capacity + the ≤50% anti-bridging cap, **PX-10** the arm gate, **PX-13** the two-drum reaction). The proportional gain (damping 0.7) is **[CALIB]** — AutoDig's PID gains are not published — so acceptance is on the closed-loop BEHAVIOUR, never on reproducing NASA's gains. **PRIOR ART, stated so it does not overclaim:** NTRS 20210022218 (ICE-RASSOR *learning excavation*) is NASA replacing the AutoDig PID trencher with deep RL; STEWIE cites the MASS paper (…781), not that one. [REQ:PX-14] `stewie/runtime/test_viz2_autodig.py` (7 gates) on the REAL Haworth SfS bundle: the runtime exposes `drum_current_a`/`drum_mass_inferred_kg`/`should_offload` and the current RISES with fill; the inferred mass tracks the true mass within the FDC band; the closed loop regulates the achieved ingestion to the setpoint (open-loop cuts the max bite regardless); a higher setpoint takes bigger bites and fills in fewer passes; it terminates on full without overfilling **under FDC-half-full sensor noise**; the regulated bite respects the PX bounds and a stowed arm still cuts nothing. Real TDD (RED first). **NON-VACUOUS, proven two ways:** neutering the feedback (always max depth) turns the three regulation/setpoint gates RED; neutering the offload guard turns the no-overfill gate RED. I=D (the sensor wired into telemetry + `autodig_trench` closed loop + per-pass depth override), X=D (the drum-current observable is now live telemetry on the public console), V=D. (needs PX-13; unblocks TR-02 — the last physics row before the interpretable policy) | D | D | D | NA |
| GW-13 | P1 | **viz2's Godot render surface must honour the GW-12 local-render-origin rule (it does not).** GW-12 requires every render surface to declare a local origin near the work area and render float32-relative to it, with the largest coordinate handed to a float32 path under a documented budget (**ULP < 1 cm**). The /ide and cockpit surfaces comply (0.98 mm, 8.69x margin). **viz2 does not:** `state_fields.world_min` is the bundle's real IAU_2015:30135 corner and `_pose_x = world_min.x + col*cell_m` flows straight into float32 Node3D transforms, so the render step is SITE-DEPENDENT. Two bundles viz2 actually SERVES from `/bundles` BREACH the bound: **malapert_massif_10km_5m (|132 km| -> 15.62 mm)** and **nobile_rim2_10km_5m (|164 km| -> 15.62 mm)** -- coarser than the sim's own 13.2 mm anti-bridging bite -- while nine more sit at 1.28-2.56x margin. Nothing in GW-12's own test covers viz2's Godot path, so this shipped unnoticed. SEVERITY, honestly: this is a RENDER-precision defect, not a physics one. The authoritative pose is float64 in the python runtime (which is what the HUD reports), Godot never sends a world coordinate back, and both breaching bundles are 5 m-cell DEMs -- so 15.6 mm is invisible today. It is nonetheless a real breach of a documented contract, and it gets worse with any site further from the projection origin. FIX (designed; ATTEMPTED AND REVERTED -- see below): rebase the Godot scene to a local origin (keep the true corner as `render_origin_m`, set `world_min` to zero) so scene coordinates span only the SITE EXTENT (<= ~20 km -> ULP <= 1.2 mm; 0.12 mm over a 2 km window), clearing every site with 8x+ margin. Sound in principle because the Godot frame is render-only. **A first attempt was reverted:** terrain and rover rendered correctly, the parse gate passed, and the HUD coordinate stayed exactly right -- but the ENTIRE ROCK FIELD vanished, because the clasts arrive from the runtime's `clasts.json` in absolute world metres and did not end up in the local frame. Neither the parse gate nor the HUD caught it; only a before/after render CAPTURE did. So the retry must (a) first establish frame-ownership for EVERY world-coordinate ingest (clasts.json, the `{plan:{route}}` push, the lander), and (b) be gated on a screenshot diff, not just a parse gate. This is a live PUBLIC render surface (RT-06), so the retry wants a human in the loop. [REQ:GW-13] `stewie/dataset/test_float32_render_bound.py` is a CANARY, not an endorsement: it pins the exact breaching set, so adding a site further out FAILS immediately (an even coarser render cannot ship silently), and landing the render-origin fix ALSO fails it (the set empties) -- forcing whoever fixes it to come back and assert the empty set. It also proves the fix is sufficient: render-origin-relative, every site clears the budget. Non-vacuous (verified to fail on a changed set). I=N (the render origin is not implemented), X=N, V=P (the breach is MEASURED and GUARDED; the requirement is not met). (gap in GW-12's coverage; bounds RT-06) | N | N | P | NA |
| LY-05 | P1 | **DEM-derivative analysis rasters: aspect, curvature, roughness drape, contours.** Aspect (gradient azimuth) + curvature (profile/plan or Laplacian, stated) join `_GLOBE_KINDS` as real producers with legend entries computed from the same heightfield gradient the slope drape uses; roughness is exposed as a standalone drape (same window-RMS-slope definition as lode/costmap_layers._roughness, one source of truth); contours generate as a REAL vector product (QGIS-Desktop gdal_contour layers persisted into the .qgz at stated intervals, and/or a backend GeoJSON endpoint) registered in the LY-01 catalog with provenance + eligibility (display-only by default). Acceptance: each new kind renders via `/layers/globe/{kind}.png` + bbox on a real site, appears in the /ide layer tree with a legend, and a test asserts aspect/curvature values on a real-DEM fixture crop. [REQ:LY-05] `stewie/server/test_ly05_dem_derivatives.py` verifies all four on the REAL LOLA Haworth 5 m DEM crop (relief std 103.85 m, no synthetic data): aspect_deg + curvature_laplacian are computed from the SAME numpy.gradient the slope drape uses (the aspect field points downhill on 99.99% of interior sloped cells; the Laplacian ∇²z sign matches concave-up(+)/convex-up(-), corr -0.88 vs centre-minus-neighbour, low-point +0.032 / high-point -0.0004), the roughness drape colours lode.costmap_layers._roughness (the ONE window-RMS-slope source of truth, imported not re-derived), aspect/curvature/roughness each render a non-degenerate PNG via `/layers/globe/{kind}.png` + bbox on haworth while the allow-list still 404s an unknown kind, `/layers/legend` carries all three ramps, and `/layers/contours.geojson` is a real contourpy isoline product reprojected to selenographic lon/lat in the Haworth footprint at the stated interval (osgeo/gdal is absent in this venv, so the GeoJSON endpoint is the PRD's named alternative to gdal_contour-into-.qgz). I=D (the four producers live in gis_layers.py: aspect_deg/curvature_laplacian/_roughness_rgba + contour_geojson), X=D (registered in the LY-01 catalog as terrain.aspect/terrain.curvature/terrain.roughness/base.contours, drape in /viz + the /ide layer tree via catalogLayers.js, and carry /layers/legend entries), V=D (the [REQ:LY-05] test asserts the aspect azimuth + curvature sign on the real-DEM crop, not just array shapes). (extends LY-01/GW-06; producers land in stewie/server/gis_layers.py + gis/build_project.py) | D | D | D | NA |
...
| LY-07 | P1 | **Terrain-change / dig-fill-depth drape (before-vs-after DEM difference).** A signed elevation-difference drape (base DEM vs the as-built/observed compose from stewie/twin/terrain_view.compose_terrain_view) renders cut (below base) vs fill (above base) with a diverging legend and per-cell depth readout via `/world/point`; zero-change is transparent; the drape carries the as_built/twin versions it was computed from. This is the visual producer for the catalog rows map.changed_terrain + evidence.before_after_dem. Acceptance: after a conserved cut+fill transaction on a real site, the drape shows the cut region negative and the berm positive with depths matching the transaction volumes; [REQ:LY-07] test. [REQ:LY-07] `stewie/server/test_ly07_changed_terrain_drape.py` proves the drape on the REAL LOLA Haworth 5 m DEM (no synthetic terrain): a conserved cut+fill SIM run (`POST /executive/run` folds the mass-conserving cut/fill into the site TerrainMemory, the SD-01 path) makes the signed as-built-minus-base difference show the cut region NEGATIVE (red) and the berm POSITIVE (blue) with the deepest cut equal to the authority's max_cut_m and the imprinted net volume equal to the conserved net_volume_m3, an unworked cell fully transparent, and /world/point reporting the SAME per-cell depth (runtime_evidence.as_built_delta_m). I=D (the producer lives in stewie/server/gis_layers.py: `_changed_terrain_rgba` + `_render_globe_changed_terrain`, a diverging red↔blue ramp about zero, zero-change transparent), X=D (registered as the `changed_terrain` globe kind in `_GLOBE_KINDS` with a `/layers/legend` entry, bound to the LY-01 catalog rows map.changed_terrain + evidence.before_after_dem, the render carrying the as_built/twin versions it was computed from), V=D (the [REQ:LY-07] test asserts cut-negative/berm-positive with depths matching the transaction + the per-cell /world/point readout, not just array shapes). (extends LY-01/DT-04/SD-01; data path already exists at /world/terrain_view + /dem/asbuilt) | D | D | D | NA |
| QG-04 | P2 | **QGIS Server WFS + WMTS proven (completes QG-02's claim).** The --profile gis QGIS Server serves (a) WFS: GetCapabilities lists the project's site vectors and a GetFeature returns them in IAU_2015:30135; (b) WMTS: GetCapabilities advertises a lunar-CRS tile matrix and a GetTile returns a correct pole-truthful tile, or the WMTS leg is explicitly recorded infeasible on the pinned server version with the reason (like the Trek WMTS deferral). gis/test_server.py extends to assert both (skip-clean when no server is up). QG-02's shipped evidence is WMS-only (gis/SERVER.md); this row makes the WFS/WMTS words in QG-02 true or honestly retracts them. (extends QG-02; alternative raster path already tracked as TT-01) | N | N | N | NA |

### 7.F AUTODIG excavation autonomy + layered sensor ingest (2026-07-08 fold)

Aaron's AUTODIG (autonomous excavation, NOT road-vehicle Autoware) + sensor-ingest architecture
(design/STEWIE_autodig_ingest_design_2026-07-08.md). AUTODIG is the TRL-5 behavior layer; motor
current/energy are PROXY observables for effective regolith resistance (never a direct density
measurement); live robot telemetry ingests through a gatekept ROS2 pipeline as observations-with-
uncertainty into the versioned mutable world model. Lane keys: AD AUTODIG excavation + effort-proxy;
IN sensor ingest + world-model tiers. Most of the reconciliation spine already EXISTS (MP-11 residual ->
EG-08 lifecycle -> hash-chained TwinStore/TerrainMemory); these rows add the excavation control + inverse
effort-proxy + live ingest + tier enforcement that are genuinely new.

| ID | P | Requirement and acceptance | I | X | V | Q |
| --- | --- | --- | --- | --- | --- | --- |
| AD-01 | P1 | **AUTODIG behavior layer.** An autonomous-excavation controller (drum torque = process variable, arm/bite-depth = control variable, per docs/vehicle_ipex.md) sequences dig/bite/drum-speed/lift/dump/traverse beneath lode/berm_fsm; torque regulates to a setpoint across regolith densities, the cut-depth bound is respected, a stall aborts, the offload handoff fires, and it emits the AD-02 effort log. (extends berm_fsm + arm_state + WorkSite) [REQ:AD-01] `lode/test_autodig.py` (10 gates: torque regulates to setpoint across regolith densities — denser→shallower bite, the density adaptation PX-14 could not claim; cut-depth bound; rock/overload/tip aborts to STALL; drum-full fires the offload handoff to berm_fsm; per-pass conserved-effort record). Built as a controller lib on the real conserved authority; NOT yet wired to a live surface, so X=N. | D | N | D | NA |
| AD-02 | P1 | **Per-actuator electrical observables (I, V, tau, P, E).** Excavation/drive actuators synthesize current/voltage/torque/power/energy from the conserved effort (generalizing the DrumSensor FDC model), published on the validated proprioception power channel and truth-firewalled; two regolith densities produce distinguishable logs whose energy reconciles with dig_energy_per_kg. (extends rassor_mass_model + twin.proprioception; needs AD-01) | N | N | N | NA |
| AD-03 | P1 | **Effective-regolith-state estimator (inverse effort-proxy).** The predicted-vs-observed excavation-effort residual yields per-cell OBSERVED rho_eff/R_dig/k_terrain/E_specific with uncertainty via MP-11 proposals into EG-08; typed proxy=True, NEVER surfaced as measured density; a DISTINCT belief-layer product from the existing FORWARD excavation_resistance drape (whose mislabel is tracked by the physics council, task #53), reconciled against the forward prediction; a seeded dense pocket yields a localized accepted update the next plan re-costs over. (extends reconciliation_step + EG-08 + LY-01; needs AD-01/02) | N | N | N | NA |
...
| IN-02 | P1 | **Enforced raw/derived/belief/world/mission layer taxonomy.** A closed tier in {raw, derived, belief, world, mission} on every LY-01 catalog entry + twin surface (beside source_class); planning/release require world/mission (or belief carrying uncertainty), and promotion only advances raw->derived->belief->world through EG-08; a raw-tier layer marked planning-valid fails validation. (extends LY-01 + GW-03 + EG-08) | N  [REQ:IN-02] BUILT + VERIFIED (2026-07-16 fan-out): a closed tier in {raw,derived,belief,world,mission} on every LY-01 catalog entry + twin surface; planning/release require world/mission (or belief carrying uncertainty); promotion only advances raw->derived->belief->world->mission. stewie/server/layer_tier.py + enforcement wired into the world router; test_layer_tier_enforcement.py (12, re-verified on main, exit 0). | D | D | D | NA |
...
| DA-04 | P1 | **Durable edit sessions (GW-08 -> persistent store).** The GW-08 keep-out/mission-feature edit session moves off the process-memory `_SESSIONS` dict into the durable store (SQLite fallback or PostGIS), so an in-progress edit session + its versioned before/after audit + undo SURVIVE a backend restart; a session created, mutated, then reloaded after a simulated restart replays identically. (extends stewie/server/edit_session.py; the clearest persistence defect) GLYPH-LAG: already ships — `edit_session.py` write-throughs every mutation to `db.py` (async SQLAlchemy: PostGIS via $STEWIE_DATABASE_URL, per-$STEWIE_DATA_DIR SQLite fallback), and `get_session` reloads durable rows on a cache miss. [REQ:DA-04] `stewie/server/test_da04_durable_edit_session.py` (create→mutate→drop cache = simulated restart→reload replays version, keep-outs, and the whole audit trail byte-for-byte; a post-reload edit + undo persist across a second restart). | D | D | D | NA |
...
| TW-12 | P1 | A viewshed/LOS layer producer fills terrain.los: an r.viewshed-class viewshed from the site DEM + a localization anchor writes the terrain.los raster that SN-05's visibility term consumes (today SN-05 marches dart.visibility.is_visible per-route with no terrain.los producer). (extends SN-05) [REQ:TW-12] `dart/test_viewshed.py`: `dart/visibility.py` fills `terrain.los` from the site DEM + a localization anchor, and `dart/illumination_cost.py` (SN-05) consumes it. Internal cost-term producer, not a public surface, so X=N. | D | N | D | NA |
...
| CP-09 | P1 | Construction actions mutate `WorldState`; routing, illumination, observability, and acceptance consume the updated terrain. Three consumers read the MUTATED as-built in the product `/plan` path: routing (`run_closed_loop` re-hazards a leg crossing a freshly EXECUTED cut/fill at the repose slope -- `test_berm_rehazard`), acceptance (`validate_plan` flatness/berm/repose on `as_built` -- CP-06), and illumination (shadow recompute after excavation -- TW-08 `test_shadow_predict`). Observability now CONSUMES the as-built in the live loop: at the dig gate `run_closed_loop` (opt-in `dense_perception`) renders the executed as-built, scores observed-vs-belief, and DEFERS/re-surveys on a perceived self-made hazard (`test_dense_perception_loop` -- verified on a CI real-render fixture AND a live on-host render). Remaining (X partial): per-mission local-patch extraction from an arbitrary DEM + default-on integration. | D | P | D | NA |
...
| FL-08 | P1 | A Fleet SCADA supervisor dashboard shows per-rover live status/battery/drum-fill(bucket)/health/ETA from real PlanResult/runtime telemetry; a test asserts each rover row binds real per-vehicle state (no global-inventory fallback). (extends FL-04/PO-11) | N | N | N | NA |
...
| PO-09 | P1 | Mission/profile schemas are versioned and migratable. | P  [REQ:PO-09] VERIFIED (P->D, 2026-07-16 fan-out): the mission wire-format is now versioned + migratable with a REAL cross-version round-trip on a real committed fixture (sample_missions/01_flatten_pad.json, v0->stewie_mission/1.0 via a non-empty migrator registry wired into mission_from_dict); the profile migrator registry was already built + tested. HONEST: the profile cross-version DEMO stays blocked (the profile schema has only ever been 1.0, and fabricating a legacy one is forbidden) -- so the round-trip acceptance is discharged on the mission half, which uses real data. test_mission_migration.py (6, main exit 0). | D | D | D | NA |
...
| GI-03 | P2 | GIS interoperability scope: define and implement the mission-required subset only -- GeoJSON/COG import, selected OGC/ArcGIS service consumption, feature attributes/query, measurement/profile tools, provenance, and offline mission package export. Do not claim ArcGIS parity. | D | D | P | NA |
...
| FS-10 | P1 | Optimization budgets: define and enforce latency, memory, CPU/GPU, bandwidth, tile/cache, and model-inference budgets for map rendering, planning, fleet solving, Navigation estimation, and cockpit mobile performance. | P  [REQ:FS-10] PARTIAL (2026-07-16 fan-out): the LATENCY/tile-cache budget is now enforced by a REAL live producer (per-route latency middleware + percentiles + breach warning on /metrics; test_perf_budgets.py re-verified on main). V stays P: the row names SIX budget classes (latency, memory, CPU/GPU, bandwidth, tile/cache, model-inference) and only latency/tile-cache is wired; the rest remain. | P | P | P | N |
...
| AS-04 | P0 | ROS container tiers: provide reproducible containers for base ROS2 Jazzy dev, perception/SLAM, Gazebo simulation, RViz diagnostics, bridge runtime, and a Space ROS migration profile. Each container has a smoke command and pinned package manifest. | D  [REQ:AS-04] `scripts/test_as04_container_tiers.py` asserts all six tier Dockerfiles are STRUCTURALLY reproducible (pinned base, package manifest, smoke command) -- a real guard on X. V stays P HONESTLY: the release gate's container-evidence ledger records a build+smoke for only 3 of the 6 tiers (base/rviz/gazebo = AS-02/05/06); the perception/SLAM, bridge-runtime, and Space ROS tiers are NOT yet built (a Dockerfile existing is not the image building), so V=D waits on recorded container runs for those three. The base tier's build IS recorded (RT-00). | D | D | P | NA |
...
| FS-25 | P1 | **Product mode + runnable profile in the route/state model (review finding 4).** The cockpit route/state model carries the PRD product mode (`GIS-PLAN`/`TRAIN`/`SIM-OPERATE`/`EVALUATE`/future `OPERATE`) and the runnable profile (`desktop_sil`/`digital_twin`/`ros2_replay`/`hil_jetson`/`sensor_bench`/`rover_bench`/`field_traverse`/`monte_carlo`) as first-class routeable, persisted, shareable fields — beyond today's `source`(live/sim/eval)+`mode`(sandbox/live) (`cockpit_state.js:8`). The persistent mode/profile rail (§26.2) is driven from this state; sim/forecast/replay/HIL/live never share ambiguous styling. Acceptance: a shared link restores mode+profile; the rail reflects them on every screen. Buildable on archimedes. PARTIAL: the backend route/state CONTRACT landed — `stewie/contracts/route_state.py` makes the 5 product modes × 8 runnable profiles a strict, frozen, versioned, fail-closed pydantic contract that validates + round-trips at the route boundary ([REQ:FS-25] `stewie/contracts/test_route_state.py`). But the row's stated E2E acceptance (a shared link restores mode+profile; the rail reflects them on every screen) is a FRONTEND behavior and is NOT verified by this backend contract, so V stays N and I=P (backend anchor only). | P | N | N | NA |
...
| FS-31 | P1 | Execute-and-watch mission SSE stream + resume: GET /executive/run/{id}/stream emits text/event-stream legs each carrying id: <leg>; a reconnecting EventSource sending Last-Event-ID replays ONLY legs after that id. [REQ:FS-31] `stewie/server/test_executive_stream.py` (4 real-run functional tests, no mocks): GET /executive/run/{id}/stream returns text/event-stream, replays each persisted leg as an SSE event carrying `id: <leg>` then a terminal done, 404s an unknown run, requires operator auth, and — the resume clause — a reconnecting request sending Last-Event-ID:"0" replays ONLY the legs after leg 0 (leg 0 skipped, exactly one fewer event). I=D (executive.py::executive_run_stream: per-leg `id: <i>` SSE + Last-Event-ID→resume_after skip, one-way push, owner-scoped), X=D (the Execute pane plays a run back via EventSource; the tests run in the CI python suite), V=D (the [REQ:FS-31] test proves the id-carrying stream + the Last-Event-ID replay-after semantics on a REAL persisted run). [SHIPPED: stewie/server/routers/executive.py:378 + #58.2] | D | D | D | NA |
| DT-06 | P1 | Twin dirty-read consistency under concurrent resync: a /world read concurrent with a /resync write returns one consistent (mask, heights, version) triple, never a torn read mixing pre-/post-resync state. (extends DT-03) [REQ:DT-06] `stewie/server/test_dt06_resync_consistency.py` (the WRITE half + same-lock proof) + `stewie/server/test_current_terrain_view.py`::reads_the_twin_under_the_resync_lock (the READ half): the no-torn-read guarantee is a single mutex state._RESYNC_LOCK held by BOTH sides — the observed-twin read samples (mask, heights, version) as one triple under the lock (functionally spied: locked==True during the read), and the /twin/resync WRITE holds the SAME lock across apply_patch..world-commit..compensate (functionally spied: locked==True during both the mutation and the world-state commit), so a /world read concurrent with a resync sees pre- or post- state, never a torn mix. I=D (state.py:86 read + routers/twin.py:76 write both `with state._RESYNC_LOCK`, one critical section each), X=D (the read is state.current_terrain_view's twin overlay; the write is the operator-gated POST /twin/resync; both run in the CI python suite), V=D (the [REQ:DT-06] tests prove BOTH sides hold the lock during their critical section + that it is the same module-level mutex). [SHIPPED: stewie/server/state.py _RESYNC_LOCK] | D | D | D | NA |
| FS-32 | P1 | A RESEARCH/OPERATE/TRAIN persona-lens as a routeable+persisted field over ONE world model: switching re-foregrounds researcher/operator/ML-engineer control clusters WITHOUT changing selected mission/world/authority; a shared link restores the lens; a test asserts the lens changes foregrounded panels but not world/authority state. (extends FS-25/FR-01 + EG-04) | N | N | N | NA |
...
| BA-12 | P0 | **The Gazebo terrain is RESOLVABLE AT ROVER SCALE and reads the conserved authority's own DEM.** BA-04 put a real lunar DEM under the Gazebo rover instead of a flat plane — real progress — but it shipped an **8-bit greyscale PNG**, and **its gate asserted the wrong thing**: `assert arr.min() == 0 and arr.max() == 255, "heightmap is flat / not full-range"`. That "proves" the terrain is not flat by asserting **the 8-bit range is fully used** — i.e. it asserts the very quantisation that made the world unusable. BA-04 was glyphed done, its test was green, and **the terrain could not resolve the machine standing on it.** It tested the ARTEFACT, not the REQUIREMENT. **MEASURED, what the old world actually was:** 513×513 px over 2565 m → **5.00 m/pixel**; 8-bit over 933.761 m of relief → **3.662 m per vertical STEP**. Against the machine that must drive on it: IPEx drum footprint 0.3526 m = **1/14 of ONE pixel**; anti-bridging dig pass 0.0132 m = **1/277 of ONE vertical step**; IPEx obstacle spec 0.075 m = **1/49 of a step**; the rover's own wheel radius 0.1524 m = **1/24 of a step**. At rover scale it was a smooth blob — no rocks, no obstacles, no ruts, no berms. That does not merely block excavation, **it breaks the perception and navigation the Gazebo world exists to serve**: an Autoware stack driving a featureless 5 m/px surface has nothing to perceive. **AND THE PREMISE WAS FALSE.** The generator's docstring asserted the 8-bit encoding was forced — *"gz-sim reads an 8-bit grayscale heightmap"*. **Verified wrong in our own container** (`stewie-gazebo:jazzy`): `libgz-common5-geospatial.so.5.8.0` → `ldd` shows **`libgdal.so.34`**, and `gz/common/geospatial/Dem.hh` declares `class Dem : public HeightmapData` with `double Elevation(...)`. **gz-sim reads a GeoTIFF DEM directly, with float elevation.** The quantisation was self-inflicted. **AND A SECOND TRUTH, WORSE THAN THE QUANTISATION:** the generator sourced `haworth_10km_5m` (5 m cell) while viz2 and the conserved authority drive on `haworth_sfs_2km_1m` (1 m cell) — **Gazebo's terrain was not even the same DEM.** Under the architecture decision (the conserved authority is the SINGLE physics authority; Gazebo is a ROS sensor shell that CONSUMES its raster — one truth, two engines), two truths mean evidence gathered in one engine does not transfer to the other. Result — **horizontal 5.00 m → 1.00 m; vertical 3662 mm → 1.91 µm; source → the authority's own bundle.** A dig pass went from **1/277 of a step to 6,921× the step**; the 7.5 cm obstacle from 1/49 to **39,322×**; the rover's wheel from 1/24 to **79,954×**. **TWO DEFECTS FOUND ONLY BY LOADING THE WORLD IN THE CONTAINER — the file was perfectly correct and the world was broken.** (1) For a DEM heightmap **gz uses the raster's RAW values on the z axis** (taking only x/y from `<size>`), so raw lunar elevations floated the terrain **1.7 km above the world origin** (`ODE Heightfield AABB … max = {256, 256, 1764.56}`) and the rover would have spawned underneath it → the DEM is now **ZERO-BASED** (a LOCAL worksite frame, the same discipline as GW-12's local render origin) with the absolute datum **recorded, not lost** (`z + datum_m` recovers true elevation). Verified: `AABB min = {-256,-256,-0.05} max = {256,256,16.646}`. (2) **gz can NEVER auto-size a lunar DEM:** `Dem` computes extent by transforming corners to **EPSG:4326 (Earth)**, and PROJ correctly refuses — *"Source and target ellipsoid do not belong to the same celestial body (Moon vs Earth)"* — so **`<size>` is LOAD-BEARING, not decorative**, and nothing in the code said so. PROJ offers `PROJ_IGNORE_CELESTIAL_BODY=YES`; **we deliberately DO NOT set it** — it would make PROJ treat the Moon as Earth and compute the extent against Earth's radius, a wrong number that looks right, which is worse than a loud fallback to a value we control. **WHAT THIS DOES NOT CLAIM:** horizontal resolution is bounded by the DEM itself. You cannot resolve a 7.5 cm obstacle from a 1 m raster, and inventing sub-metre terrain would be fabrication. **Rocks and boulders belong as CLAST COLLISION MODELS** (the sourced Golombek draw the authority already carries), never baked into a heightmap — no 1 m raster can represent a 0.3 m boulder, and pretending otherwise is how you get a world that looks detailed and teaches a perception stack nothing. That is the next row, not this one. [REQ:BA-12] `scripts/test_gazebo_dem_fidelity.py` (7 gates) + the corrected [REQ:BA-04] gate (its 0..255 assertion is gone; the fidelity requirement now lives where it belongs). Real TDD (RED first). VERIFIED IN-CONTAINER on `stewie-gazebo:jazzy`, not merely on disk. I=D, X=D, V=D. (fixes the gate of BA-04; unblocks the Autoware / EZ-RASSOR perception + mapping path) | D | D | D | NA |
| BA-13 | P0 | **The Gazebo world carries the authority's own ROCKS as real collision geometry — so there is finally something to perceive.** BA-12 made the terrain resolvable at rover scale, but terrain alone is a **bare graded surface**: nothing to see, nothing to hit. An Autoware stack driving a rock-free plain learns nothing about lunar navigation, and perception is the entire reason a Gazebo world exists here. **ROCKS ARE OBJECTS, NOT HEIGHTMAP DETAIL.** At the DEM's 1 m cell a 0.29 m boulder is **a third of one pixel** — a raster physically cannot represent it, and smearing it into the terrain would produce a world that LOOKS detailed and teaches a perception stack nothing. So they enter as objects: collision + visual geometry, at real positions, with real radii, from the **sourced Golombek size-frequency draw the conserved authority already carries** (same `world_seed` → **the same rocks Gazebo and viz2 both see**: one truth, two engines; TR-01 made that seed live). **MEASURED population** over BA-12's 513 m terrain window (seed 0): **67,721 clasts**, radius **min 12.8 cm, median 15.8 cm, max 29.2 cm** — **every one larger than IPEx's 7.5 cm obstacle spec**, so no size threshold can honestly thin them; they all matter. 67,721 SDF *models* would be 67,721 entities and would kill the sim, so: they go into **ONE STATIC model with N `<collision>`/`<visual>` spheres in ONE link** (cheap for ODE — static geometry, no dynamics, no per-entity bookkeeping), and the field is scoped to the **rover's operating area** (128 m) rather than the full terrain extent. **NO SILENT CAP:** the provenance and the SDF header both state **4,184 emitted vs 67,721 in the terrain window** — the remainder are *out of scope, not missing*. Silent truncation reads as "we modelled everything" when we did not. **THE FRAME BUG THIS ROW EXISTS TO NOT SHIP.** The authority emits `center_m = [x, ELEVATION, y]` — **Godot's Y-UP** — while Gazebo is **Z-UP**, and BA-12 additionally **zero-based** the terrain against a ~1748 m datum. Get the axis swap or the datum wrong and every rock floats in the sky or sinks out of sight — **and from a distance it would still look like a rock field.** So the gate does not count rocks; it asserts **each rock sits on the ground the world actually loads**: sample the exported raster under the rock's (x,y) and require `centre_z == terrain_z + r·(1 − 2·buried_frac)` (the authority's own placement law; `buried_frac 0.5` = exactly half-buried). **NON-VACUOUS, proven three ways** — dropping the datum, swapping the Y-up axes, and mirroring the row axis each turn it RED. [REQ:BA-13] `scripts/test_gazebo_rockfield.py` (5 gates). Real TDD. **VERIFIED IN-CONTAINER, from gz's OWN ENTITY TREE** (`stewie-gazebo:jazzy`), not from the file on disk: `gz sim` loads and steps the world (`GZ_EXIT=0`), `gz model --list` reports `rockfield`, and gz's server-side world regeneration echoes **8,368 `<sphere>` = 4,184 `<collision>` + 4,184 `<visual>`** — every rock is in the physics engine. **HONEST GAP:** a *rendered camera frame* of the rock field was NOT captured (the container is llvmpipe-only — GPU render in Docker is blocked), so the geometry is verified numerically against gz's entity tree and the raster, but not yet by eye. I=D, X=D, V=D. (needs BA-12; unblocks the Autoware / EZ-RASSOR perception path — the rover can now SEE and HIT real lunar boulders) | D | D | D | NA |
