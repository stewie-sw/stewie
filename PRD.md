...
| TW-04 | P1 | One seeded composite generator combines craters, rocks, material, and illumination parameters. `build_from_dem` currently combines craters (DEM corridor) + material (ChaSTE density) + illumination, but NOT rocks. The rocks-completion (a seeded Golombek boulder layer, commit `bd21352`) was attempted but REVERTED: the full-suite [REQ:TW-04] test exceeded the 600 s timeout. Diagnosed (run-verified, faulthandler stack-sampled): the cost is `populate_craters` -> `carve_crater` during fine-tile generation (`mosaic.ensure_fine` -> `_generate_fine_tile` -> `overlay_residual`), NOT rocks (bounding `sample_boulders d_min_m` 0.04->0.30 did NOT help; >90 s even WITHOUT rocks). Root cause: `carve_crater` (`stewie/terrain/procgen.py`) is O(full-grid) PER crater -- it builds full-grid `r`/`delta` arrays, calls `cs.derive_height()`, and calls `cs.set_height_via_mass()` TWICE (procgen.py:251 + :260) for every crater, while `populate_craters` carves the entire crater size-frequency distribution per fine tile -> O(n_craters x grid). **`carve_crater` is now FIXED** (commit `fe18d91`, byte-identical: windowed to `max(ej_outer, R+38.7*sigma)` -- beyond `R+38.7*sigma` the rim gaussian's `exp()` underflows to exactly 0.0 -- all 42 crater tests + the full suite pass unchanged, crater tests 5s->under 1s). With craters fixed, a SECOND bottleneck dominates: `dart/illumination.py:175 horizon_clip` (the illumination horizon scan, `build_from_dem:1070`), O(grid) over the cm-resolution fine tile. With carve_crater fixed + rocks bounded (`d_min_m` 0.30), the [REQ:TW-04] rocks test PASSES but at **551 s** (the test runs `build_from_dem(radius_m=20)` x3 for the seeded-determinism check) -- only 49 s under the 600 s CI timeout, and always-on rocks also slows the sibling `build_from_dem` tests. `stewie/terrain/test_scenes.py` [REQ:TW-04] validates the current three-family composite + provenance + datum round-trip on the real Haworth DEM (`@skipif` when the DEM bundle is absent). I=P/X=P/V=P (held): next-action FORK = (A) optimize `horizon_clip` (another careful byte-identical core pass, makes `build_from_dem` genuinely fast) then re-add rocks, OR (B) reduce the `build_from_dem` test radii for comfortable margin (test-only, validates a smaller corridor) then re-add rocks. carve_crater alone did not get the radius=20 path under a safe margin. | P | P | P | NA |
