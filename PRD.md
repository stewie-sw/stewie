...
| FS-31 | P1 | Execute-and-watch mission SSE stream + resume: GET /executive/run/{id}/stream emits text/event-stream legs each carrying id: <leg>; a reconnecting EventSource sending Last-Event-ID replays ONLY legs after that id. [SHIPPED: stewie/server/routers/executive.py:378 + #58.2] | N | N | N | NA |
| DT-06 | P1 | Twin dirty-read consistency under concurrent resync: a /world read concurrent with a /resync write returns one consistent (mask, heights, version) triple, never a torn read mixing pre-/post-resync state. (extends DT-03) [SHIPPED: stewie/server/state.py _RESYNC_LOCK] | N | N | N | NA |
