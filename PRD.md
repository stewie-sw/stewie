...
| PX-13 | P1 | **Counter-rotating both-drum dig — the chassis reaction nets ~0 (KSC-TOPS-7). This is a FIDELITY BUG, not a missing feature.** `stewie/specs/arm_state.py:126 net_dig_reaction_n` already models the defining mechanical feature of the RASSOR/IPEx lineage, citing **KSC-TOPS-7 (T2.2)**: *"counter-rotating drums dig in opposing directions — the horizontal reactions are equal and OPPOSITE, so the pair nets ~0 and a single drum nets F = τ/r."* But the live dig **does not use it**: `viz2_runtime._dig_fee` admits in-comment that *"the live model cuts with a SINGLE (front) drum, so `net_dig_reaction_n` leaves the FULL draft on the chassis (a future counter-rotating both-drum dig would net ~0) → fed to the next drive tick's traction demand."* So the simulator is shoving the rover backward with a force **the real vehicle is specifically engineered to cancel**, and that wrong force flows straight into traction, slip, and energy on EVERY dig tick. Any policy trained on today's sim learns to compensate for a reaction that does not exist on the real machine — which makes this a prerequisite for the training rows, not a polish item. Acceptance: the live dig engages BOTH drums counter-rotating; the net horizontal chassis reaction from a symmetric both-drum dig is ~0 (not F = τ/r); a deliberately asymmetric/single-drum dig still nets the full draft (the model must not just zero the term — it must zero it *for the right reason*); the traction demand on the tick after a dig reflects the corrected reaction; and the existing PX-09/PX-10/PX-11 bounds still hold per drum. Guard must be NON-VACUOUS: demonstrated to FAIL if the counter-rotation is removed. (needs PX-12; the model already exists in `arm_state.net_dig_reaction_n` and is unused on the dig path) | N | N | N | NA |
