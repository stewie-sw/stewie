...
| SD-02 | P1 | Cut/fill material-balance: for cut+fill orders materialBalance returns bank cut volume, loose spoil (RHO_DEEP->RHO_SPOIL bulking), compacted fill demand, and net borrow/spoil in kg with mass conserved (cut-only => net spoil, fill-only => net borrow). (extends SD-01) [SHIPPED: gis/qwc2/js/mission/planTools.js + planTools.test.js; densities moon-hardcoded, body-aware is task #62] | N | N | N | NA |
