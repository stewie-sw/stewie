...
| PX-14 | P1 | **AutoDig-equivalent closed-loop trench control — regulate the bite from the drum-current observable.** NASA's **AutoDig** (the semi-autonomous excavation routine for RASSOR → IPEx) closes a control loop that STEWIE does not have: it maintains ground contact and **regulates digging depth from the drum motors' current draw (torque)**, i.e. from how much soil the drums are actually ingesting. STEWIE has the SENSOR and not the LOOP. `stewie/physics/rassor_mass_model.py` already grounds the exact telemetry AutoDig closes on — the **Free-spinning Drum Current** model (mass ∝ steady avg drum current; `FDC_LINEAR_R2 = (0.989, 0.985)`, `FDC_MPE_ALL = 7.40%`, `FDC_MPE_HALF_FULL = 2.56%`), sourced from **NTRS 20210022781** (ICE-RASSOR, AIAA ASCEND 2021) — and can both synthesize the current reading from conserved drum mass and infer mass back from it with published error bars. But the live dig is a COMMANDED verb (`cmd == "dig"` → `_apply_dig()`): no controller, no feedback, no contact maintenance. Acceptance: a closed-loop trench controller drives the bite from the (optionally noisy) drum-current observable rather than from a fixed commanded depth; it holds a target ingestion rate under VARYING terrain resistance (a harder/denser cell reduces the bite, a softer one increases it) instead of blindly cutting a constant depth; it respects the already-sourced bounds (PX-09 capacity refusal + ≤50% anti-bridging bite, PX-10 arm gate, PX-12 metered discharge) rather than replacing them; it terminates a trench on the drum-full condition via the existing `should_offload` autonomy trigger; and it degrades honestly under the paper's real sensor uncertainty (planning against IMPERFECT fill knowledge, which is the entire point of the FDC model). HONEST SCOPE: the PID/gain structure is [CALIB] — the AutoDig gains are not published — so the ACCEPTANCE is on the closed-loop BEHAVIOUR (bite responds to ingestion feedback, bounds hold, terminates on full), never on reproducing NASA's specific gains, which would be fabrication. (needs PX-13 — a depth controller cannot regulate against a reaction force that is still wrong) | N | N | N | NA |
