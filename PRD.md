...
| GW-09 | P2 | A dual-mode planning graticule (selenographic lon/lat densely sampled + reprojected so it curves in polar-stereographic, plus a straight metric km-grid with labels) overlays the lunar map given an injected reproject(lon,lat)->[x,y]; a node test asserts gridline + label generation. (extends GW-05) [SHIPPED: gis/qwc2/js/mission/graticule.js + graticule.test.js] | N | N | N | NA |
