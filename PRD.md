...
| CP-04 | P1 | Goal grammar supports typed structures, tolerances, budgets, priorities, deadlines, dependencies, and keep-outs. `lode/test_mission_intent_compiler.py` [REQ:CP-04] validates the grammar (objectives->orders, hard budgets, soft-weight priorities, deadlines, prerequisites->precedence, real planner path). V held at P pending I=D (tolerances/keep-outs coverage + implementation-completeness audit); cited + under review for promotion. | P | P | P | NA |
