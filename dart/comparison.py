"""Navigation vs the LAC state of the art: a grounded comparison framework.
...
Navigation contributes what neither has: the shadow as a GEOMETRIC instrument (azimuth->heading SN-03,
...
COMPLEMENTARY -- ShadowNav coarse-global-map-match, Stanford passive-relative-VO, Navigation active-local-
...
def accuracy_precision_comparison(*, near_range_m=6.0, dh_m: float = 0.1743) -> dict:
    ...
    ShadowNav is m-level GLOBAL (absolute lunar position), Navigation is cm-to-dm LOCAL (a map-free fix);
    they are not measured on a shared testbed yet (that is the §6 protocol). Stanford + ShadowNav
    numbers are quoted from the cited papers; Navigation precision is the parallax covariance model."""
    sp = math.radians(0.05)
    L = np.array([[near_range_m, 0.0], [0.0, near_range_m], [-near_range_m + 1, -near_range_m + 2]])
    nav_sigma = AP.position_fix_sigma(L, np.array([0.0, 0.0]), dh_m=dh_m, sigma_theta_rad=sp)
    return {
        ...
        "Navigation": {
            "accuracy_m": round(nav_sigma, 3), "precision_m": round(nav_sigma, 3),
...
def operational_cost(*, n_fixes: int = 10, traverse_m: float = 100.0, dh_m: float = 0.1743,
                     vehicle_mass_kg: float = 30.0, g: float = G_MOON, nav_maneuver_s: float = 8.0,
    ...
    ``n_fixes`` fixes, on the IPEx energy budget. GROUNDED: the Navigation standstill-fix energy is the
    ...
    nav_fix_J = RM.arm_raise_lift_energy_j(vehicle_mass_kg, g, lift_height_m=dh_m)  # raise; lowering is gravity
    ...
    return {
        ...
        "Navigation": {
            "per_fix_time_s": float(nav_maneuver_s), "per_fix_distance_m": 0.0,
            "per_fix_energy_J": round(nav_fix_J, 2),
            "equiv_drive_m": round(nav_fix_J / drive_epm, 3),
            "extra_mission_energy_J": round(n_fixes * nav_fix_J, 1),
            "extra_mission_time_s": round(n_fixes * nav_maneuver_s, 1), "regime": "low sun",
...
def compare_position_fix(dem, true_rc, landmarks_xy, *, cell_m, dh_m, fx_px, sigma_px=0.3,
    ...
    (register_to_dem) vs the Navigation articulation parallax. Returns {approach: error_m, ...}. The
    ...
    # Navigation: articulation parallax -> ranges -> trilateration (map-free, heading-free)
    ...
    err_nav = float(np.hypot(fix[0] - true_xy[0], fix[1] - true_xy[1]))
    return {"ShadowNav-class (map-match)": err_shadownav, "Navigation (articulation parallax)": err_nav,
