"""[REQ:AS-09] standstill-relocalization acceptance (§25 Phase 7): an accepted articulation-
...
from dart import relocalization as ar
...
def test_accepted_standstill_fix_reduces_covariance():
    res = ar.standstill_fix(PRIOR_XY, PRIOR_COV, NEAR_WELL_SPREAD, dh_m=DH_M, fx_px=FX_PX)
...
def test_collinear_landmarks_rejected_not_inserted():
    ...
    res = ar.standstill_fix(PRIOR_XY, PRIOR_COV, collinear, dh_m=DH_M, fx_px=FX_PX)
...
def test_beyond_resolvable_range_rejected():
    ...
    res = ar.standstill_fix(PRIOR_XY, PRIOR_COV, far, dh_m=DH_M, fx_px=FX_PX)
    assert not res["accepted"]
