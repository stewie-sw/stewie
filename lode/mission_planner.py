...
# ARCH-2 (#123): the vehicle registry (V), input validation (VAL) and the body soil model
# (get_body/params_for_body) are used only by the mission DATA MODEL, now in lode.planner_model.
...
# is rebound per-vehicle from ctx inside the sims, so it is not imported at module scope here.)
from lode.planner_constants import (  # noqa: E402
    BATTERY_J, CHARGE_W, DIG_RATE_KG_S, DRIVE_J_PER_M,
    LOCALIZATION_MARGIN_M, RESERVE_FRAC, _CONSTRAINT_CAPS,
)
...
# CHARGE_W / LOCALIZATION_MARGIN_M / _CONSTRAINT_CAPS now live in lode.planner_constants (ARCH-2 #123),
# imported above; RESERVE_FRAC likewise (ARCH-03). Values + uses below are byte-identical.
